...
.popsection
   ...
   800d4:	100b06e1 	adr	x1, 961b0 <master_cpu_set>
    mov     x2, 1
...
1:
   ...
   800e8:	35ffffa3 	cbnz	w3, 800dc <_enter_el1+0x50>
    adr     x1, master_cpu
   800ec:	100b05e1 	adr	x1, 961a8 <master_cpu>
    str     x0, [x1]
...
wait_flag:
    ...
    adr x1, wait_flag
   80100:	100b05c1 	adr	x1, 961b8 <wait_flag>
    mov x2, #1
...
1:
    adr x1, wait_flag
   8010c:	100b0561 	adr	x1, 961b8 <wait_flag>
    ldr x2, [x1]
   ...
   80130:	9100003f 	mov	sp, x1
    ...
    b _init
   80134:	14000354 	b	80e84 <_init>
    b _exit
   80138:	14000342 	b	80e40 <_exit>
...
void pmu_setup(size_t start, size_t n) {
   ...
   808c8:	913d0042 	add	x2, x2, #0xf40
void pmu_setup(size_t start, size_t n) {
...
void print_samples_latency() {
   ...
   80970:	913d0318 	add	x24, x24, #0xf40
   ...
   80994:	91008301 	add	x1, x24, #0x20
...
static bool cpu_is_master() {
   ...
   80b24:	d00000a0 	adrp	x0, 96000 <JIS_state_table+0xb0>

void main(void){

    if(!cpu_is_master()) {
   80b28:	f940d400 	ldr	x0, [x0, #424]
   80b2c:	eb21001f 	cmp	x0, w1, uxtb
   80b30:	54000b61 	b.ne	80c9c <main+0x17c>  // b.any
void main(void){
   80b34:	a9bb7bfd 	stp	x29, x30, [sp, #-80]!
            for (size_t i = 0; i < L1_CACHE_SIZE; i+= CACHE_LINE_SIZE) {
   80b38:	52800c83 	mov	w3, #0x64                  	// #100
void main(void){
   80b3c:	910003fd 	mov	x29, sp
   80b40:	a9046bf9 	stp	x25, x26, [sp, #64]
   80b44:	90000419 	adrp	x25, 100000 <pmu_used_counters>
   80b48:	91000339 	add	x25, x25, #0x0
   ...
   80b64:	900000b5 	adrp	x21, 94000 <__any_on>
   80b68:	913d02b5 	add	x21, x21, #0xf40
   80b6c:	a90363f7 	stp	x23, x24, [sp, #48]
                    }
                }
   ...
   80b70:	91004338 	add	x24, x25, #0x10
            pmu_setup(i, sample_events_size - i);
   80b74:	d2800097 	mov	x23, #0x4                   	// #4
    MSR(PMCR_EL0, 0x1);
   80b78:	52800036 	mov	w22, #0x1                   	// #1
            for (size_t i = 0; i < L1_CACHE_SIZE; i+= CACHE_LINE_SIZE) {
   80b7c:	d2800142 	mov	x2, #0xa                   	// #10
   80b80:	d2800000 	mov	x0, #0x0                   	// #0
   80b84:	d503201f 	nop
                cache_l1[i] = i;
   80b88:	12001c01 	and	w1, w0, #0xff
   80b8c:	38206a61 	strb	w1, [x19, x0]
            for (size_t i = 0; i < L1_CACHE_SIZE; i+= CACHE_LINE_SIZE) {
   80b90:	91010000 	add	x0, x0, #0x40
   80b94:	f144001f 	cmp	x0, #0x100, lsl #12
   80b98:	54ffff81 	b.ne	80b88 <main+0x68>  // b.any
        for(size_t it_idx = 0; it_idx < MAX_ITER; it_idx++){
   80b9c:	f1000442 	subs	x2, x2, #0x1
   80ba0:	54ffff01 	b.ne	80b80 <main+0x60>  // b.any
    for(int warm_samp = 0; warm_samp< NUM_WARMUPS; warm_samp++)
   80ba4:	71000463 	subs	w3, w3, #0x1
   80ba8:	54fffea1 	b.ne	80b7c <main+0x5c>  // b.any
        size_t i = 0;
   80bac:	d280001a 	mov	x26, #0x0                   	// #0
    pmu_setup_counters(n, &sample_events[start]);
   80bb0:	8b1a0ea1 	add	x1, x21, x26, lsl #3
   80bb4:	cb1a02e0 	sub	x0, x23, x26
            sample_count = 0;
   80bb8:	f900073f 	str	xzr, [x25, #8]
    pmu_setup_counters(n, &sample_events[start]);
   80bbc:	97ffff11 	bl	80800 <pmu_setup_counters>
    unsigned long pmcr = MRS(PMCR_EL0);
   80bc0:	d53b9c00 	mrs	x0, pmcr_el0
    MSR(PMCR_EL0, pmcr | 0x6);
   80bc4:	b27f0400 	orr	x0, x0, #0x6
   80bc8:	d51b9c00 	msr	pmcr_el0, x0
    MSR(PMCR_EL0, 0x1);
   80bcc:	d51b9c16 	msr	pmcr_el0, x22
            while(sample_count < NUM_SAMPLES) {
   80bd0:	f9400720 	ldr	x0, [x25, #8]
   80bd4:	f1031c1f 	cmp	x0, #0xc7
   80bd8:	54000528 	b.hi	80c7c <main+0x15c>  // b.pmore
    unsigned long pmcr = MRS(PMCR_EL0);
   80bdc:	d53b9c00 	mrs	x0, pmcr_el0
    MSR(PMCR_EL0, pmcr | 0x6);
   80be0:	b27f0400 	orr	x0, x0, #0x6
   80be4:	d51b9c00 	msr	pmcr_el0, x0
    }
}
...
static inline uint64_t pmu_cycle_get(){
   ...
   80be8:	d53b9d03 	mrs	x3, pmccntr_el0
   80bec:	d2800142 	mov	x2, #0xa                   	// #10
                    for (size_t i = 0; i < L1_CACHE_SIZE; i+= CACHE_LINE_SIZE) {
   80bf0:	d2800000 	mov	x0, #0x0                   	// #0
   80bf4:	d503201f 	nop
                        cache_l1[i] = i;
   80bf8:	12001c01 	and	w1, w0, #0xff
   80bfc:	38206a61 	strb	w1, [x19, x0]
                    for (size_t i = 0; i < L1_CACHE_SIZE; i+= CACHE_LINE_SIZE) {
   80c00:	91010000 	add	x0, x0, #0x40
   80c04:	f144001f 	cmp	x0, #0x100, lsl #12
   80c08:	54ffff81 	b.ne	80bf8 <main+0xd8>  // b.any
                for(size_t it_idx = 0; it_idx < MAX_ITER; it_idx++){
   80c0c:	f1000442 	subs	x2, x2, #0x1
   80c10:	54ffff01 	b.ne	80bf0 <main+0xd0>  // b.any
   80c14:	d53b9d01 	mrs	x1, pmccntr_el0
    return (size_t) bit_extract(MRS(PMCR_EL0), PMCR_N_OFF, PMCR_N_LEN);
   80c18:	d53b9c00 	mrs	x0, pmcr_el0
   80c1c:	d34b3c04 	ubfx	x4, x0, #11, #5
    for(int i = 0; i < n; i++){
   80c20:	f275101f 	tst	x0, #0xf800
   80c24:	540001a0 	b.eq	80c58 <main+0x138>  // b.none
        pmu_samples[i][sample_count] = pmu_counter_get(i);
   80c28:	f9400726 	ldr	x6, [x25, #8]
    MSR(PMSELR_EL0, counter);
   80c2c:	d51b9ca2 	msr	pmselr_el0, x2
    return MRS(PMXEVCNTR_EL0);
   80c30:	d53b9d45 	mrs	x5, pmxevcntr_el0
   80c34:	937f7c40 	sbfiz	x0, x2, #1, #32
   80c38:	8b22c000 	add	x0, x0, w2, sxtw
   80c3c:	d37df000 	lsl	x0, x0, #3
   80c40:	8b22c000 	add	x0, x0, w2, sxtw
    for(int i = 0; i < n; i++){
   80c44:	91000442 	add	x2, x2, #0x1
        pmu_samples[i][sample_count] = pmu_counter_get(i);
   80c48:	8b000cc0 	add	x0, x6, x0, lsl #3
   80c4c:	f8207a85 	str	x5, [x20, x0, lsl #3]
    for(int i = 0; i < n; i++){
   80c50:	eb02009f 	cmp	x4, x2
   80c54:	54fffea1 	b.ne	80c28 <main+0x108>  // b.any
                exec_time_samples[sample_count] = exec_cycles;
   80c58:	f9400722 	ldr	x2, [x25, #8]
                exec_cycles = final_cycle - initial_cycle;
   80c5c:	cb030021 	sub	x1, x1, x3
                sample_count++;
   80c60:	f9400720 	ldr	x0, [x25, #8]
                exec_time_samples[sample_count] = exec_cycles;
   80c64:	f8227b01 	str	x1, [x24, x2, lsl #3]
                sample_count++;
   80c68:	91000400 	add	x0, x0, #0x1
   80c6c:	f9000720 	str	x0, [x25, #8]
            while(sample_count < NUM_SAMPLES) {
   80c70:	f9400720 	ldr	x0, [x25, #8]
   80c74:	f1031c1f 	cmp	x0, #0xc7
   80c78:	54fffb29 	b.ls	80bdc <main+0xbc>  // b.plast
    return (size_t) bit_extract(MRS(PMCR_EL0), PMCR_N_OFF, PMCR_N_LEN);
   80c7c:	d53b9c00 	mrs	x0, pmcr_el0
   80c80:	d34b3c00 	ubfx	x0, x0, #11, #5
            }
        
            i += pmu_num_counters();
   80c84:	8b00035a 	add	x26, x26, x0
            print_samples_latency();
   80c88:	97ffff1e 	bl	80900 <print_samples_latency>
        while(i < sample_events_size){
   80c8c:	f1000f5f 	cmp	x26, #0x3
   80c90:	54fff909 	b.ls	80bb0 <main+0x90>  // b.plast
            for (size_t i = 0; i < L1_CACHE_SIZE; i+= CACHE_LINE_SIZE) {
   80c94:	52800c83 	mov	w3, #0x64                  	// #100
   80c98:	17ffffb9 	b	80b7c <main+0x5c>
   80c9c:	d65f03c0 	ret

0000000000080ca0 <irq_set_handler>:
#include <stddef.h>
...
void irq_set_handler(unsigned id, irq_handler_t handler){
    if(id < IRQ_NUM)
   80ca0:	710ffc1f 	cmp	w0, #0x3ff
   80ca4:	54000088 	b.hi	80cb4 <irq_set_handler+0x14>  // b.pmore
        irq_handlers[id] = handler;
   80ca8:	90001402 	adrp	x2, 300000 <irqlat_end_samples>
   80cac:	91324042 	add	x2, x2, #0xc90
   80cb0:	f8205841 	str	x1, [x2, w0, uxtw #3]
}
   80cb4:	d65f03c0 	ret
   80cb8:	d503201f 	nop
   80cbc:	d503201f 	nop

0000000000080cc0 <irq_handle>:

void irq_handle(unsigned id){
   80cc0:	2a0003e1 	mov	w1, w0
    if(id < IRQ_NUM && irq_handlers[id] != NULL)
   80cc4:	710ffc1f 	cmp	w0, #0x3ff
   80cc8:	540000e8 	b.hi	80ce4 <irq_handle+0x24>  // b.pmore
   80ccc:	90001402 	adrp	x2, 300000 <irqlat_end_samples>
   80cd0:	91324042 	add	x2, x2, #0xc90
   80cd4:	f8615841 	ldr	x1, [x2, w1, uxtw #3]
   80cd8:	b4000061 	cbz	x1, 80ce4 <irq_handle+0x24>
        irq_handlers[id](id);
   80cdc:	aa0103f0 	mov	x16, x1
   80ce0:	d61f0200 	br	x16
}
   80ce4:	d65f03c0 	ret
	...

0000000000080cf0 <_read>:
#include <cpu.h>
#include <fences.h>
#include <wfi.h>
...
{
   80cf0:	a9bd7bfd 	stp	x29, x30, [sp, #-48]!
   80cf4:	910003fd 	mov	x29, sp
   80cf8:	f90013f5 	str	x21, [sp, #32]
   80cfc:	2a0203f5 	mov	w21, w2
    int i;
    for (i = 0; i < len; ++i)
   80d00:	7100005f 	cmp	w2, #0x0
   80d04:	5400014d 	b.le	80d2c <_read+0x3c>
   80d08:	a90153f3 	stp	x19, x20, [sp, #16]
   80d0c:	aa0103f3 	mov	x19, x1
   80d10:	8b22c034 	add	x20, x1, w2, sxtw
   80d14:	d503201f 	nop
    {
        ptr[i] = uart_getchar();
   80d18:	9400007e 	bl	80f10 <uart_getchar>
   80d1c:	38001660 	strb	w0, [x19], #1
    for (i = 0; i < len; ++i)
   80d20:	eb14027f 	cmp	x19, x20
   80d24:	54ffffa1 	b.ne	80d18 <_read+0x28>  // b.any
   80d28:	a94153f3 	ldp	x19, x20, [sp, #16]
    }

    return len;
}
   80d2c:	2a1503e0 	mov	w0, w21
   80d30:	f94013f5 	ldr	x21, [sp, #32]
   80d34:	a8c37bfd 	ldp	x29, x30, [sp], #48
   80d38:	d65f03c0 	ret
   80d3c:	d503201f 	nop

0000000000080d40 <_write>:

int _write(int file, char *ptr, int len)
{
   80d40:	a9bd7bfd 	stp	x29, x30, [sp, #-48]!
   80d44:	910003fd 	mov	x29, sp
   80d48:	a90153f3 	stp	x19, x20, [sp, #16]
   80d4c:	8b22c034 	add	x20, x1, w2, sxtw
   80d50:	f90013f5 	str	x21, [sp, #32]
   80d54:	2a0203f5 	mov	w21, w2
    int i;
    for (i = 0; i < len; ++i)
   80d58:	7100005f 	cmp	w2, #0x0
   80d5c:	5400022d 	b.le	80da0 <_write+0x60>
   80d60:	aa0103f3 	mov	x19, x1
   80d64:	14000005 	b	80d78 <_write+0x38>
   80d68:	91000673 	add	x19, x19, #0x1
    {
        ...
        uart_putc(ptr[i]);
   80d6c:	94000066 	bl	80f04 <uart_putc>
    for (i = 0; i < len; ++i)
   80d70:	eb14027f 	cmp	x19, x20
   80d74:	54000160 	b.eq	80da0 <_write+0x60>  // b.none
        if (ptr[i] == '\n')
   80d78:	39400260 	ldrb	w0, [x19]
   80d7c:	7100281f 	cmp	w0, #0xa
   80d80:	54ffff41 	b.ne	80d68 <_write+0x28>  // b.any
            uart_putc('\r');
   80d84:	528001a0 	mov	w0, #0xd                   	// #13
   80d88:	9400005f 	bl	80f04 <uart_putc>
        uart_putc(ptr[i]);
   80d8c:	39400260 	ldrb	w0, [x19]
    for (i = 0; i < len; ++i)
   80d90:	91000673 	add	x19, x19, #0x1
        uart_putc(ptr[i]);
   80d94:	9400005c 	bl	80f04 <uart_putc>
    for (i = 0; i < len; ++i)
   80d98:	eb14027f 	cmp	x19, x20
   80d9c:	54fffee1 	b.ne	80d78 <_write+0x38>  // b.any
    }

    return len;
}
   80da0:	a94153f3 	ldp	x19, x20, [sp, #16]
   80da4:	2a1503e0 	mov	w0, w21
   80da8:	f94013f5 	ldr	x21, [sp, #32]
   80dac:	a8c37bfd 	ldp	x29, x30, [sp], #48
   80db0:	d65f03c0 	ret

0000000000080db4 <_lseek>:

int _lseek(int file, int ptr, int dir)
{
   80db4:	a9bf7bfd 	stp	x29, x30, [sp, #-16]!
   80db8:	910003fd 	mov	x29, sp
    errno = ESPIPE;
   80dbc:	94000491 	bl	82000 <__errno>
   80dc0:	aa0003e1 	mov	x1, x0
   80dc4:	528003a2 	mov	w2, #0x1d                  	// #29
    return -1;
}
   80dc8:	12800000 	mov	w0, #0xffffffff            	// #-1
    errno = ESPIPE;
   80dcc:	b9000022 	str	w2, [x1]
}
   80dd0:	a8c17bfd 	ldp	x29, x30, [sp], #16
   80dd4:	d65f03c0 	ret
   80dd8:	d503201f 	nop
   80ddc:	d503201f 	nop

0000000000080de0 <_close>:

int _close(int file)
{
    return -1;
}
   80de0:	12800000 	mov	w0, #0xffffffff            	// #-1
   80de4:	d65f03c0 	ret
   80de8:	d503201f 	nop
   80dec:	d503201f 	nop

0000000000080df0 <_fstat>:

int _fstat(int file, struct stat *st)
{
    st->st_mode = S_IFCHR;
   80df0:	52840002 	mov	w2, #0x2000                	// #8192
    return 0;
}
   80df4:	52800000 	mov	w0, #0x0                   	// #0
    st->st_mode = S_IFCHR;
   80df8:	b9000422 	str	w2, [x1, #4]
}
   80dfc:	d65f03c0 	ret

0000000000080e00 <_isatty>:

int _isatty(int fd)
{
   80e00:	a9bf7bfd 	stp	x29, x30, [sp, #-16]!
   80e04:	910003fd 	mov	x29, sp
    errno = ENOTTY;
   80e08:	9400047e 	bl	82000 <__errno>
   80e0c:	aa0003e1 	mov	x1, x0
   80e10:	52800322 	mov	w2, #0x19                  	// #25
    return 0;
}
   80e14:	52800000 	mov	w0, #0x0                   	// #0
    errno = ENOTTY;
   80e18:	b9000022 	str	w2, [x1]
}
   80e1c:	a8c17bfd 	ldp	x29, x30, [sp], #16
   80e20:	d65f03c0 	ret

0000000000080e24 <_sbrk>:
...
void* _sbrk(int increment)
{
   ...
   80e24:	d00000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
{
   80e28:	2a0003e1 	mov	w1, w0
    char* current_heap_end = heap_end;
   80e2c:	f940c840 	ldr	x0, [x2, #400]
    heap_end += increment;
   80e30:	8b21c001 	add	x1, x0, w1, sxtw
   80e34:	f900c841 	str	x1, [x2, #400]
    return current_heap_end;
}
   80e38:	d65f03c0 	ret
   80e3c:	d503201f 	nop

0000000000080e40 <_exit>:
    DMB(ish);
   80e40:	d5033bbf 	dmb	ish
   80e44:	d503201f 	nop
#ifndef WFI_H
#define WFI_H
...
static inline void wfi(){
    asm volatile("wfi\n\t" ::: "memory");
   80e48:	d503207f 	wfi
   80e4c:	17ffffff 	b	80e48 <_exit+0x8>

0000000000080e50 <_getpid>:
}
...
int _getpid(void)
{
  return 1;
   ...
   80e50:	52800020 	mov	w0, #0x1                   	// #1
   80e54:	d65f03c0 	ret
   80e58:	d503201f 	nop
   80e5c:	d503201f 	nop

0000000000080e60 <_kill>:

int _kill(int pid, int sig)
{
   80e60:	a9bf7bfd 	stp	x29, x30, [sp, #-16]!
   80e64:	910003fd 	mov	x29, sp
    errno = EINVAL;
   80e68:	94000466 	bl	82000 <__errno>
   80e6c:	aa0003e1 	mov	x1, x0
   80e70:	528002c2 	mov	w2, #0x16                  	// #22
    return -1;
}
   80e74:	12800000 	mov	w0, #0xffffffff            	// #-1
    errno = EINVAL;
   80e78:	b9000022 	str	w2, [x1]
}
   80e7c:	a8c17bfd 	ldp	x29, x30, [sp], #16
   80e80:	d65f03c0 	ret

0000000000080e84 <_init>:
...
__attribute__((weak))
void _init(){
   80e84:	a9bd7bfd 	stp	x29, x30, [sp, #-48]!
static inline void spin_lock(spinlock_t* lock){
    ...
    asm volatile (
   80e88:	d0001400 	adrp	x0, 302000 <irq_handlers+0x1370>
   80e8c:	52800021 	mov	w1, #0x1                   	// #1
   80e90:	910003fd 	mov	x29, sp
   80e94:	f9000bf3 	str	x19, [sp, #16]
   80e98:	91324013 	add	x19, x0, #0xc90
   80e9c:	885ffe62 	ldaxr	w2, [x19]
   80ea0:	35ffffe2 	cbnz	w2, 80e9c <_init+0x18>
   80ea4:	88027e61 	stxr	w2, w1, [x19]
   80ea8:	35ffffa2 	cbnz	w2, 80e9c <_init+0x18>

    spin_lock(&init_lock);
    if(!init_done) {
   80eac:	39401260 	ldrb	w0, [x19, #4]
   80eb0:	b9002fe2 	str	w2, [sp, #44]
   80eb4:	340000a0 	cbz	w0, 80ec8 <_init+0x44>
...
static inline void spin_unlock(spinlock_t* lock){
   ...
   80eb8:	889ffe7f 	stlr	wzr, [x19]
    ...
    arch_init();
   80ebc:	940000e5 	bl	81250 <arch_init>

    int ret = main();
   80ec0:	97ffff18 	bl	80b20 <main>
    _exit(ret);
   80ec4:	97ffffdf 	bl	80e40 <_exit>
        init_done = true;
   80ec8:	39001261 	strb	w1, [x19, #4]
        uart_init();
   80ecc:	94000005 	bl	80ee0 <uart_init>
   80ed0:	17fffffa 	b	80eb8 <_init+0x34>
	...

0000000000080ee0 <uart_init>:
...
void uart_init(){
   ...
   80ee0:	52984002 	mov	w2, #0xc200                	// #49664
   80ee4:	5298d801 	mov	w1, #0xc6c0                	// #50880
   80ee8:	d28a0800 	mov	x0, #0x5040                	// #20544
   80eec:	72a00022 	movk	w2, #0x1, lsl #16
   80ef0:	72a005a1 	movk	w1, #0x2d, lsl #16
   80ef4:	f2bfc420 	movk	x0, #0xfe21, lsl #16
   80ef8:	52800084 	mov	w4, #0x4                   	// #4
   80efc:	52800003 	mov	w3, #0x0                   	// #0
   80f00:	1400008c 	b	81130 <uart8250_init>

0000000000080f04 <uart_putc>:
}

void uart_putc(char c)
{
    uart8250_putc(c);
   80f04:	14000023 	b	80f90 <uart8250_putc>
   80f08:	d503201f 	nop
   80f0c:	d503201f 	nop

0000000000080f10 <uart_getchar>:
}

char uart_getchar(void)
{
   80f10:	a9bf7bfd 	stp	x29, x30, [sp, #-16]!
   80f14:	910003fd 	mov	x29, sp
    return uart8250_getc();
   80f18:	9400003e 	bl	81010 <uart8250_getc>
}
   80f1c:	a8c17bfd 	ldp	x29, x30, [sp], #16
   80f20:	d65f03c0 	ret

0000000000080f24 <uart_enable_rxirq>:

void uart_enable_rxirq()
{
    uart8250_enable_rx_int();
   80f24:	14000070 	b	810e4 <uart8250_enable_rx_int>
   80f28:	d503201f 	nop
   80f2c:	d503201f 	nop

0000000000080f30 <uart_clear_rxirq>:
}

void uart_clear_rxirq()
{
    uart8250_interrupt_handler(); 
   80f30:	14000055 	b	81084 <uart8250_interrupt_handler>
	...

0000000000080f40 <get_reg>:
...
static volatile u32 get_reg(u32 num)
{
   ...
   80f40:	d0001401 	adrp	x1, 302000 <irq_handlers+0x1370>
   80f44:	91326022 	add	x2, x1, #0xc98
   80f48:	b94c9821 	ldr	w1, [x1, #3224]

	if (uart8250_reg_width == 1)
   80f4c:	b9400443 	ldr	w3, [x2, #4]
		return readb(uart8250_base + offset);
   80f50:	f9400442 	ldr	x2, [x2, #8]
	u32 offset = num << uart8250_reg_shift;
   80f54:	1ac12004 	lsl	w4, w0, w1
		return readb(uart8250_base + offset);
   80f58:	1ac12000 	lsl	w0, w0, w1
	if (uart8250_reg_width == 1)
   80f5c:	7100047f 	cmp	w3, #0x1
   80f60:	54000120 	b.eq	80f84 <get_reg+0x44>  // b.none
	else if (uart8250_reg_width == 2)
   80f64:	7100087f 	cmp	w3, #0x2
   80f68:	54000060 	b.eq	80f74 <get_reg+0x34>  // b.none
		return readw(uart8250_base + offset);
	else
		return readl(uart8250_base + offset);
   80f6c:	b8607840 	ldr	w0, [x2, x0, lsl #2]
}
   80f70:	d65f03c0 	ret
		return readw(uart8250_base + offset);
   80f74:	d37ff800 	lsl	x0, x0, #1
   80f78:	78606840 	ldrh	w0, [x2, x0]
   80f7c:	12003c00 	and	w0, w0, #0xffff
}
   80f80:	d65f03c0 	ret
		return readb(uart8250_base + offset);
   80f84:	38644840 	ldrb	w0, [x2, w4, uxtw]
   80f88:	12001c00 	and	w0, w0, #0xff
}
   80f8c:	d65f03c0 	ret

0000000000080f90 <uart8250_putc>:
	u32 offset = num << uart8250_reg_shift;
   80f90:	d0001403 	adrp	x3, 302000 <irq_handlers+0x1370>
   80f94:	91326061 	add	x1, x3, #0xc98
   80f98:	528000a2 	mov	w2, #0x5                   	// #5
	else
...
{
   80f9c:	12001c00 	and	w0, w0, #0xff
	u32 offset = num << uart8250_reg_shift;
   80fa0:	b94c9863 	ldr	w3, [x3, #3224]
		return readl(uart8250_base + offset);
   80fa4:	f9400425 	ldr	x5, [x1, #8]
	if (uart8250_reg_width == 1)
   80fa8:	b9400421 	ldr	w1, [x1, #4]
		return readl(uart8250_base + offset);
   80fac:	1ac32042 	lsl	w2, w2, w3
		return readb(uart8250_base + offset);
   80fb0:	8b0200a6 	add	x6, x5, x2
		return readl(uart8250_base + offset);
   80fb4:	8b0208a4 	add	x4, x5, x2, lsl #2
		return readw(uart8250_base + offset);
   80fb8:	8b0204a3 	add	x3, x5, x2, lsl #1
	if (uart8250_reg_width == 1)
   80fbc:	7100043f 	cmp	w1, #0x1
   80fc0:	540000a1 	b.ne	80fd4 <uart8250_putc+0x44>  // b.any
		return readb(uart8250_base + offset);
   80fc4:	394000c1 	ldrb	w1, [x6]
	while ((get_reg(UART_LSR_OFFSET) & UART_LSR_THRE) == 0)
   80fc8:	362fffe1 	tbz	w1, #5, 80fc4 <uart8250_putc+0x34>
		writeb(val, uart8250_base + offset);
   80fcc:	390000a0 	strb	w0, [x5]
		;
   ...
   80fd4:	7100083f 	cmp	w1, #0x2
   80fd8:	54000120 	b.eq	80ffc <uart8250_putc+0x6c>  // b.none
		return readl(uart8250_base + offset);
   80fdc:	b9400082 	ldr	w2, [x4]
	while ((get_reg(UART_LSR_OFFSET) & UART_LSR_THRE) == 0)
   80fe0:	362fffa2 	tbz	w2, #5, 80fd4 <uart8250_putc+0x44>
	if (uart8250_reg_width == 1)
   80fe4:	7100043f 	cmp	w1, #0x1
   80fe8:	54ffff20 	b.eq	80fcc <uart8250_putc+0x3c>  // b.none
	else if (uart8250_reg_width == 2)
   80fec:	7100083f 	cmp	w1, #0x2
   80ff0:	540000a0 	b.eq	81004 <uart8250_putc+0x74>  // b.none
		writel(val, uart8250_base + offset);
   80ff4:	b90000a0 	str	w0, [x5]
}
   80ff8:	d65f03c0 	ret
		return readw(uart8250_base + offset);
   80ffc:	79400061 	ldrh	w1, [x3]
	while ((get_reg(UART_LSR_OFFSET) & UART_LSR_THRE) == 0)
   81000:	362fffe1 	tbz	w1, #5, 80ffc <uart8250_putc+0x6c>
		writew(val, uart8250_base + offset);
   81004:	790000a0 	strh	w0, [x5]
}
   81008:	d65f03c0 	ret
   8100c:	d503201f 	nop

0000000000081010 <uart8250_getc>:
	u32 offset = num << uart8250_reg_shift;
   81010:	b0001401 	adrp	x1, 302000 <irq_handlers+0x1370>
   81014:	91326022 	add	x2, x1, #0xc98
   81018:	528000a0 	mov	w0, #0x5                   	// #5
   8101c:	b94c9821 	ldr	w1, [x1, #3224]
	if (uart8250_reg_width == 1)
   81020:	b9400443 	ldr	w3, [x2, #4]
		return readb(uart8250_base + offset);
   81024:	f9400442 	ldr	x2, [x2, #8]
	u32 offset = num << uart8250_reg_shift;
   81028:	1ac12004 	lsl	w4, w0, w1
		return readb(uart8250_base + offset);
   8102c:	1ac12000 	lsl	w0, w0, w1
	if (uart8250_reg_width == 1)
   81030:	7100047f 	cmp	w3, #0x1
   81034:	540001a0 	b.eq	81068 <uart8250_getc+0x58>  // b.none
	else if (uart8250_reg_width == 2)
   81038:	7100087f 	cmp	w3, #0x2
   8103c:	540000a0 	b.eq	81050 <uart8250_getc+0x40>  // b.none
		return readl(uart8250_base + offset);
   81040:	b8607840 	ldr	w0, [x2, x0, lsl #2]

int uart8250_getc(void)
{
	if (get_reg(UART_LSR_OFFSET) & UART_LSR_DR)
   81044:	360001c0 	tbz	w0, #0, 8107c <uart8250_getc+0x6c>
		return readl(uart8250_base + offset);
   81048:	b9400040 	ldr	w0, [x2]
		return get_reg(UART_RBR_OFFSET);
	return -1;
}
   8104c:	d65f03c0 	ret
		return readw(uart8250_base + offset);
   81050:	d37ff800 	lsl	x0, x0, #1
   81054:	78606840 	ldrh	w0, [x2, x0]
	if (get_reg(UART_LSR_OFFSET) & UART_LSR_DR)
   81058:	36000120 	tbz	w0, #0, 8107c <uart8250_getc+0x6c>
		return readw(uart8250_base + offset);
   8105c:	79400040 	ldrh	w0, [x2]
   81060:	12003c00 	and	w0, w0, #0xffff
}
   81064:	d65f03c0 	ret
		return readb(uart8250_base + offset);
   81068:	38644840 	ldrb	w0, [x2, w4, uxtw]
	if (get_reg(UART_LSR_OFFSET) & UART_LSR_DR)
   8106c:	36000080 	tbz	w0, #0, 8107c <uart8250_getc+0x6c>
		return readb(uart8250_base + offset);
   81070:	39400040 	ldrb	w0, [x2]
   81074:	12001c00 	and	w0, w0, #0xff
}
   81078:	d65f03c0 	ret
	return -1;
   8107c:	12800000 	mov	w0, #0xffffffff            	// #-1
}
   81080:	d65f03c0 	ret

0000000000081084 <uart8250_interrupt_handler>:
	if (uart8250_reg_width == 1)
   81084:	b0001400 	adrp	x0, 302000 <irq_handlers+0x1370>
   81088:	91326000 	add	x0, x0, #0xc98

void uart8250_interrupt_handler(){
   8108c:	d10043ff 	sub	sp, sp, #0x10
	if (uart8250_reg_width == 1)
   81090:	b9400401 	ldr	w1, [x0, #4]
		return readb(uart8250_base + offset);
   81094:	f9400400 	ldr	x0, [x0, #8]
	if (uart8250_reg_width == 1)
   81098:	7100043f 	cmp	w1, #0x1
   8109c:	540001a0 	b.eq	810d0 <uart8250_interrupt_handler+0x4c>  // b.none
	else if (uart8250_reg_width == 2)
   810a0:	7100083f 	cmp	w1, #0x2
   810a4:	540000c0 	b.eq	810bc <uart8250_interrupt_handler+0x38>  // b.none
		return readl(uart8250_base + offset);
   810a8:	b9400000 	ldr	w0, [x0]
	volatile char c = get_reg(UART_RBR_OFFSET);
   810ac:	12001c00 	and	w0, w0, #0xff
   810b0:	39003fe0 	strb	w0, [sp, #15]
}
   810b4:	910043ff 	add	sp, sp, #0x10
   810b8:	d65f03c0 	ret
		return readw(uart8250_base + offset);
   810bc:	79400000 	ldrh	w0, [x0]
	volatile char c = get_reg(UART_RBR_OFFSET);
   810c0:	12001c00 	and	w0, w0, #0xff
   810c4:	39003fe0 	strb	w0, [sp, #15]
}
   810c8:	910043ff 	add	sp, sp, #0x10
   810cc:	d65f03c0 	ret
		return readb(uart8250_base + offset);
   810d0:	39400000 	ldrb	w0, [x0]
   810d4:	12001c00 	and	w0, w0, #0xff
	volatile char c = get_reg(UART_RBR_OFFSET);
   810d8:	39003fe0 	strb	w0, [sp, #15]
}
   810dc:	910043ff 	add	sp, sp, #0x10
   810e0:	d65f03c0 	ret

00000000000810e4 <uart8250_enable_rx_int>:
	u32 offset = num << uart8250_reg_shift;
   810e4:	b0001400 	adrp	x0, 302000 <irq_handlers+0x1370>
   810e8:	91326003 	add	x3, x0, #0xc98
   810ec:	52800021 	mov	w1, #0x1                   	// #1
   810f0:	b94c9800 	ldr	w0, [x0, #3224]
	if (uart8250_reg_width == 1)
   810f4:	b9400462 	ldr	w2, [x3, #4]
		writeb(val, uart8250_base + offset);
   810f8:	f9400463 	ldr	x3, [x3, #8]
	u32 offset = num << uart8250_reg_shift;
   810fc:	1ac02024 	lsl	w4, w1, w0
		writeb(val, uart8250_base + offset);
   81100:	1ac02020 	lsl	w0, w1, w0
	if (uart8250_reg_width == 1)
   81104:	6b01005f 	cmp	w2, w1
   81108:	540000e0 	b.eq	81124 <uart8250_enable_rx_int+0x40>  // b.none
	else if (uart8250_reg_width == 2)
   8110c:	7100085f 	cmp	w2, #0x2
   81110:	54000060 	b.eq	8111c <uart8250_enable_rx_int+0x38>  // b.none
		writel(val, uart8250_base + offset);
   81114:	b8207861 	str	w1, [x3, x0, lsl #2]
...
void uart8250_enable_rx_int(){
   ...
   81118:	d65f03c0 	ret
		writew(val, uart8250_base + offset);
   8111c:	78244861 	strh	w1, [x3, w4, uxtw]
}
   81120:	d65f03c0 	ret
		writeb(val, uart8250_base + offset);
   81124:	38244862 	strb	w2, [x3, w4, uxtw]
}
   81128:	d65f03c0 	ret
   8112c:	d503201f 	nop

0000000000081130 <uart8250_init>:

int uart8250_init(unsigned long base, u32 in_freq, u32 baudrate, u32 reg_shift,
		  u32 reg_width)
{
   81130:	a9bf7bfd 	stp	x29, x30, [sp, #-16]!
	u16 bdiv;

	uart8250_base	   = (volatile void *)base;
   81134:	b0001401 	adrp	x1, 302000 <irq_handlers+0x1370>
   81138:	91326026 	add	x6, x1, #0xc98
{
   8113c:	910003fd 	mov	x29, sp
   81140:	aa0003e5 	mov	x5, x0
	uart8250_reg_shift = reg_shift;
   81144:	b90c9823 	str	w3, [x1, #3224]
	u32 offset = num << uart8250_reg_shift;
   81148:	52800028 	mov	w8, #0x1                   	// #1
   8114c:	52800067 	mov	w7, #0x3                   	// #3
   81150:	528000e0 	mov	w0, #0x7                   	// #7
   81154:	52800042 	mov	w2, #0x2                   	// #2
   81158:	52800081 	mov	w1, #0x4                   	// #4
	uart8250_reg_width = reg_width;
   8115c:	b90004c4 	str	w4, [x6, #4]
	uart8250_base	   = (volatile void *)base;
   81160:	f90004c5 	str	x5, [x6, #8]
	u32 offset = num << uart8250_reg_shift;
   81164:	1ac3204d 	lsl	w13, w2, w3
   81168:	1ac3202c 	lsl	w12, w1, w3
   8116c:	1ac3210e 	lsl	w14, w8, w3
   81170:	1ac320e9 	lsl	w9, w7, w3
   81174:	1ac3200b 	lsl	w11, w0, w3
		writeb(val, uart8250_base + offset);
   81178:	1ac32042 	lsl	w2, w2, w3
   8117c:	1ac32021 	lsl	w1, w1, w3
   81180:	1ac320ea 	lsl	w10, w7, w3
   81184:	1ac32006 	lsl	w6, w0, w3
   81188:	1ac32103 	lsl	w3, w8, w3
	if (uart8250_reg_width == 1)
   8118c:	6b08009f 	cmp	w4, w8
   81190:	540003e0 	b.eq	8120c <uart8250_init+0xdc>  // b.none
	else if (uart8250_reg_width == 2)
   81194:	7100089f 	cmp	w4, #0x2
   81198:	540001e0 	b.eq	811d4 <uart8250_init+0xa4>  // b.none
		writel(val, uart8250_base + offset);
   8119c:	b82378bf 	str	wzr, [x5, x3, lsl #2]
   811a0:	52801000 	mov	w0, #0x80                  	// #128
   811a4:	b82a78a0 	str	w0, [x5, x10, lsl #2]
	/* Enable FIFO */
   ...
   811a8:	528000a0 	mov	w0, #0x5                   	// #5
		writel(val, uart8250_base + offset);
   811ac:	b82a78a7 	str	w7, [x5, x10, lsl #2]
   811b0:	b82278a8 	str	w8, [x5, x2, lsl #2]
   811b4:	b82178bf 	str	wzr, [x5, x1, lsl #2]
	get_reg(UART_LSR_OFFSET);
   811b8:	97ffff62 	bl	80f40 <get_reg>
	/* Read receive buffer */
	get_reg(UART_RBR_OFFSET);
   811bc:	52800000 	mov	w0, #0x0                   	// #0
   811c0:	97ffff60 	bl	80f40 <get_reg>
		writel(val, uart8250_base + offset);
   811c4:	b82678bf 	str	wzr, [x5, x6, lsl #2]
	/* Set scratchpad */
	set_reg(UART_SCR_OFFSET, 0x00);

	return 0;
   811c8:	52800000 	mov	w0, #0x0                   	// #0
   811cc:	a8c17bfd 	ldp	x29, x30, [sp], #16
   811d0:	d65f03c0 	ret
		writew(val, uart8250_base + offset);
   811d4:	782e48bf 	strh	wzr, [x5, w14, uxtw]
   811d8:	52801000 	mov	w0, #0x80                  	// #128
   811dc:	782948a0 	strh	w0, [x5, w9, uxtw]
	get_reg(UART_LSR_OFFSET);
   811e0:	528000a0 	mov	w0, #0x5                   	// #5
		writew(val, uart8250_base + offset);
   811e4:	782948a7 	strh	w7, [x5, w9, uxtw]
   811e8:	782d48a8 	strh	w8, [x5, w13, uxtw]
   811ec:	782c48bf 	strh	wzr, [x5, w12, uxtw]
	get_reg(UART_LSR_OFFSET);
   811f0:	97ffff54 	bl	80f40 <get_reg>
	get_reg(UART_RBR_OFFSET);
   811f4:	52800000 	mov	w0, #0x0                   	// #0
   811f8:	97ffff52 	bl	80f40 <get_reg>
		writew(val, uart8250_base + offset);
   811fc:	782b48bf 	strh	wzr, [x5, w11, uxtw]
   81200:	52800000 	mov	w0, #0x0                   	// #0
   81204:	a8c17bfd 	ldp	x29, x30, [sp], #16
   81208:	d65f03c0 	ret
		writeb(val, uart8250_base + offset);
   8120c:	382e48bf 	strb	wzr, [x5, w14, uxtw]
   81210:	12800fe0 	mov	w0, #0xffffff80            	// #-128
   81214:	382948a0 	strb	w0, [x5, w9, uxtw]
	get_reg(UART_LSR_OFFSET);
   81218:	528000a0 	mov	w0, #0x5                   	// #5
		writeb(val, uart8250_base + offset);
   8121c:	382948a7 	strb	w7, [x5, w9, uxtw]
   81220:	382d48a4 	strb	w4, [x5, w13, uxtw]
   81224:	382c48bf 	strb	wzr, [x5, w12, uxtw]
	get_reg(UART_LSR_OFFSET);
   81228:	97ffff46 	bl	80f40 <get_reg>
	get_reg(UART_RBR_OFFSET);
   8122c:	52800000 	mov	w0, #0x0                   	// #0
   81230:	97ffff44 	bl	80f40 <get_reg>
		writeb(val, uart8250_base + offset);
   81234:	382b48bf 	strb	wzr, [x5, w11, uxtw]
   81238:	52800000 	mov	w0, #0x0                   	// #0
   8123c:	a8c17bfd 	ldp	x29, x30, [sp], #16
   81240:	d65f03c0 	ret
	...

0000000000081250 <arch_init>:
#include <sysregs.h>
...
__attribute__((weak))
void arch_init(){
   81250:	a9be7bfd 	stp	x29, x30, [sp, #-32]!
   81254:	910003fd 	mov	x29, sp
   81258:	a90153f3 	stp	x19, x20, [sp, #16]
    uint64_t cpuid = MRS(MPIDR_EL1);
   8125c:	d53800b3 	mrs	x19, mpidr_el1
    uint64_t cpuid = get_cpuid();
    gic_init();
   81260:	94000084 	bl	81470 <gic_init>
    TIMER_FREQ = MRS(CNTFRQ_EL0);
   81264:	d53be001 	mrs	x1, cntfrq_el0
   81268:	b0001400 	adrp	x0, 302000 <irq_handlers+0x1370>
   8126c:	f9065401 	str	x1, [x0, #3240]
   81270:	d53800a1 	mrs	x1, mpidr_el1
    return get_cpuid() == master_cpu;
   81274:	b00000a0 	adrp	x0, 96000 <JIS_state_table+0xb0>
#ifndef SINGLE_CORE
    if(cpu_is_master()){
   81278:	f940d400 	ldr	x0, [x0, #424]
   8127c:	eb21001f 	cmp	x0, w1, uxtb
   81280:	540000a0 	b.eq	81294 <arch_init+0x44>  // b.none
        do {
   ...
   81284:	d50342ff 	msr	daifclr, #0x2
}
   81288:	a94153f3 	ldp	x19, x20, [sp, #16]
   8128c:	a8c27bfd 	ldp	x29, x30, [sp], #32
   81290:	d65f03c0 	ret
    return cpuid & MPIDR_CPU_MASK;
   81294:	92401e73 	and	x19, x19, #0xff
            ret = psci_cpu_on(i, (uintptr_t) _start, 0);
   81298:	f0fffff4 	adrp	x20, 80000 <_start>
        size_t i = cpuid + 1;
   8129c:	91000673 	add	x19, x19, #0x1
            ret = psci_cpu_on(i, (uintptr_t) _start, 0);
   812a0:	91000294 	add	x20, x20, #0x0
   812a4:	d503201f 	nop
   812a8:	aa1303e0 	mov	x0, x19
   812ac:	aa1403e1 	mov	x1, x20
        } while(i++, ret == PSCI_E_SUCCESS);
   812b0:	91000673 	add	x19, x19, #0x1
            ret = psci_cpu_on(i, (uintptr_t) _start, 0);
   812b4:	d2800002 	mov	x2, #0x0                   	// #0
   812b8:	94000022 	bl	81340 <psci_cpu_on>
        } while(i++, ret == PSCI_E_SUCCESS);
   812bc:	34ffff60 	cbz	w0, 812a8 <arch_init+0x58>
    asm volatile("MSR   DAIFClr, #2\n\t");
   812c0:	d50342ff 	msr	daifclr, #0x2
}
   812c4:	a94153f3 	ldp	x19, x20, [sp, #16]
   812c8:	a8c27bfd 	ldp	x29, x30, [sp], #32
   812cc:	d65f03c0 	ret

00000000000812d0 <smc_call>:
...
00000000000812e0 <psci_version>:
	register uint64_t r0 asm("r0") = x0;
   812e0:	d2b08000 	mov	x0, #0x84000000            	// #2214592512
	register uint64_t r1 asm("r1") = x1;
   812e4:	d2800001 	mov	x1, #0x0                   	// #0
	register uint64_t r2 asm("r2") = x2;
   812e8:	d2800002 	mov	x2, #0x0                   	// #0
    asm volatile(
   812ec:	d4000003 	smc	#0x0
--------------------------------- */
...
uint64_t psci_version(void)
{
   ...
   812f0:	93407c00 	sxtw	x0, w0
   812f4:	d65f03c0 	ret
   812f8:	d503201f 	nop
   812fc:	d503201f 	nop

0000000000081300 <psci_cpu_suspend>:


uint64_t psci_cpu_suspend(uint64_t power_state, uintptr_t entrypoint, 
                    uint64_t context_id)
{
   81300:	aa0003e3 	mov	x3, x0
	register uint64_t r0 asm("r0") = x0;
   81304:	d2800020 	mov	x0, #0x1                   	// #1
{
   81308:	aa0103e2 	mov	x2, x1
	register uint64_t r0 asm("r0") = x0;
   8130c:	f2b88000 	movk	x0, #0xc400, lsl #16
	register uint64_t r1 asm("r1") = x1;
   81310:	aa0303e1 	mov	x1, x3
    asm volatile(
   81314:	d4000003 	smc	#0x0
    return smc_call(PSCI_CPU_SUSPEND_AARCH64, power_state, entrypoint, 
                                                                    context_id);
}
   81318:	93407c00 	sxtw	x0, w0
   8131c:	d65f03c0 	ret

0000000000081320 <psci_cpu_off>:
	register uint64_t r0 asm("r0") = x0;
   81320:	d2800040 	mov	x0, #0x2                   	// #2
	register uint64_t r1 asm("r1") = x1;
   81324:	d2800001 	mov	x1, #0x0                   	// #0
	register uint64_t r0 asm("r0") = x0;
   81328:	f2b08000 	movk	x0, #0x8400, lsl #16
	register uint64_t r2 asm("r2") = x2;
   8132c:	d2800002 	mov	x2, #0x0                   	// #0
    asm volatile(
   81330:	d4000003 	smc	#0x0

uint64_t psci_cpu_off(void)
{
    return smc_call(PSCI_CPU_OFF, 0, 0, 0);
}
   81334:	93407c00 	sxtw	x0, w0
   81338:	d65f03c0 	ret
   8133c:	d503201f 	nop

0000000000081340 <psci_cpu_on>:

uint64_t psci_cpu_on(uint64_t target_cpu, uintptr_t entrypoint, 
                    uint64_t context_id)
{
   81340:	aa0003e3 	mov	x3, x0
	register uint64_t r0 asm("r0") = x0;
   81344:	d2800060 	mov	x0, #0x3                   	// #3
{
   81348:	aa0103e2 	mov	x2, x1
	register uint64_t r0 asm("r0") = x0;
   8134c:	f2b88000 	movk	x0, #0xc400, lsl #16
	register uint64_t r1 asm("r1") = x1;
   81350:	aa0303e1 	mov	x1, x3
    asm volatile(
   81354:	d4000003 	smc	#0x0
    return smc_call(PSCI_CPU_ON_AARCH64, target_cpu, entrypoint, context_id);
}
   81358:	93407c00 	sxtw	x0, w0
   8135c:	d65f03c0 	ret

0000000000081360 <psci_affinity_info>:

uint64_t psci_affinity_info(uint64_t target_affinity, 
                            uint64_t lowest_affinity_level)
{
   81360:	aa0003e3 	mov	x3, x0
	register uint64_t r0 asm("r0") = x0;
   81364:	d2800080 	mov	x0, #0x4                   	// #4
{
   81368:	aa0103e2 	mov	x2, x1
	register uint64_t r0 asm("r0") = x0;
   8136c:	f2b88000 	movk	x0, #0xc400, lsl #16
	register uint64_t r1 asm("r1") = x1;
   81370:	aa0303e1 	mov	x1, x3
    asm volatile(
   81374:	d4000003 	smc	#0x0
    return smc_call(PSCI_AFFINITY_INFO_AARCH64, target_affinity, 
                    lowest_affinity_level, 0);
}
   81378:	93407c00 	sxtw	x0, w0
   8137c:	d65f03c0 	ret

0000000000081380 <irq_enable>:
...
void irq_enable(unsigned id) {
   81380:	a9be7bfd 	stp	x29, x30, [sp, #-32]!
   gic_set_enable(id, true); 
   81384:	52800021 	mov	w1, #0x1                   	// #1
void irq_enable(unsigned id) {
   81388:	910003fd 	mov	x29, sp
   8138c:	f9000bf3 	str	x19, [sp, #16]
   gic_set_enable(id, true); 
   81390:	2a0003f3 	mov	w19, w0
   81394:	aa1303e0 	mov	x0, x19
   81398:	94000046 	bl	814b0 <gic_set_enable>
   if(GIC_VERSION == GICV2) {
       gic_set_trgt(id, gic_get_trgt(id) | (1 << get_cpuid()));
   8139c:	aa1303e0 	mov	x0, x19
   813a0:	94000060 	bl	81520 <gic_get_trgt>
   813a4:	12001c02 	and	w2, w0, #0xff
    uint64_t cpuid = MRS(MPIDR_EL1);
   813a8:	d53800a3 	mrs	x3, mpidr_el1
   813ac:	aa1303e0 	mov	x0, x19
   813b0:	52800021 	mov	w1, #0x1                   	// #1
   } else {
       gic_set_route(id, get_cpuid());
   }
}
   813b4:	f9400bf3 	ldr	x19, [sp, #16]
       gic_set_trgt(id, gic_get_trgt(id) | (1 << get_cpuid()));
   813b8:	1ac32021 	lsl	w1, w1, w3
}
   813bc:	a8c27bfd 	ldp	x29, x30, [sp], #32
       gic_set_trgt(id, gic_get_trgt(id) | (1 << get_cpuid()));
   813c0:	2a010041 	orr	w1, w2, w1
   813c4:	14000048 	b	814e4 <gic_set_trgt>
   813c8:	d503201f 	nop
   813cc:	d503201f 	nop

00000000000813d0 <irq_set_prio>:

void irq_set_prio(unsigned id, unsigned prio){
    gic_set_prio(id, (uint8_t) prio);
   813d0:	2a0003e0 	mov	w0, w0
   813d4:	14000068 	b	81574 <gic_set_prio>
   813d8:	d503201f 	nop
   813dc:	d503201f 	nop

00000000000813e0 <irq_send_ipi>:
}

void irq_send_ipi(uint64_t target_cpu_mask) {
   813e0:	a9be7bfd 	stp	x29, x30, [sp, #-32]!
   813e4:	910003fd 	mov	x29, sp
   813e8:	a90153f3 	stp	x19, x20, [sp, #16]
   813ec:	aa0003f4 	mov	x20, x0
   813f0:	d2800013 	mov	x19, #0x0                   	// #0
   813f4:	14000004 	b	81404 <irq_send_ipi+0x24>
    for(int i = 0; i < sizeof(target_cpu_mask)*8; i++) {
   813f8:	91000673 	add	x19, x19, #0x1
   813fc:	f101027f 	cmp	x19, #0x40
   81400:	54000120 	b.eq	81424 <irq_send_ipi+0x44>  // b.none
        if(target_cpu_mask & (1ull << i)) {
   81404:	9ad32681 	lsr	x1, x20, x19
   81408:	3607ff81 	tbz	w1, #0, 813f8 <irq_send_ipi+0x18>
            gic_send_sgi(i, IPI_IRQ_ID);
   8140c:	aa1303e0 	mov	x0, x19
   81410:	d2800001 	mov	x1, #0x0                   	// #0
    for(int i = 0; i < sizeof(target_cpu_mask)*8; i++) {
   81414:	91000673 	add	x19, x19, #0x1
            gic_send_sgi(i, IPI_IRQ_ID);
   81418:	9400004e 	bl	81550 <gic_send_sgi>
    for(int i = 0; i < sizeof(target_cpu_mask)*8; i++) {
   8141c:	f101027f 	cmp	x19, #0x40
   81420:	54ffff21 	b.ne	81404 <irq_send_ipi+0x24>  // b.any
        }
    }
}
   81424:	a94153f3 	ldp	x19, x20, [sp, #16]
   81428:	a8c27bfd 	ldp	x29, x30, [sp], #32
   8142c:	d65f03c0 	ret

0000000000081430 <gicc_init>:
    ...
    gicc->PMR = -1;
   81430:	b00000a0 	adrp	x0, 96000 <JIS_state_table+0xb0>
   81434:	12800002 	mov	w2, #0xffffffff            	// #-1
    gicc->CTLR = GICC_CTLR_EN_BIT;
   81438:	52800021 	mov	w1, #0x1                   	// #1
    gicc->PMR = -1;
   8143c:	f940cc00 	ldr	x0, [x0, #408]
   81440:	b9000402 	str	w2, [x0, #4]
    gicc->CTLR = GICC_CTLR_EN_BIT;
...
//        gicd->ICFGR[i] = 0xAAAAAAAA;
   ...
   81454:	52800021 	mov	w1, #0x1                   	// #1
    return ((gicd->TYPER & BIT_MASK(GICD_TYPER_ITLINENUM_OFF, GICD_TYPER_ITLINENUM_LEN) >>
   81458:	f940d000 	ldr	x0, [x0, #416]
   8145c:	b9400402 	ldr	w2, [x0, #4]
    gicd->CTLR = GICD_CTLR_EN_BIT;
   81460:	b9000001 	str	w1, [x0]
}
   81464:	d65f03c0 	ret
   81468:	d503201f 	nop
   8146c:	d503201f 	nop

0000000000081470 <gic_init>:
   81470:	d53800a1 	mrs	x1, mpidr_el1
    return cpuid & MPIDR_CPU_MASK;
   81474:	b00000a0 	adrp	x0, 96000 <JIS_state_table+0xb0>

void gic_init() {
    if(get_cpuid() == 0) {
   81478:	72001c3f 	tst	w1, #0xff
   8147c:	540000c1 	b.ne	81494 <gic_init+0x24>  // b.any
    return ((gicd->TYPER & BIT_MASK(GICD_TYPER_ITLINENUM_OFF, GICD_TYPER_ITLINENUM_LEN) >>
   81480:	91066001 	add	x1, x0, #0x198
    gicd->CTLR = GICD_CTLR_EN_BIT;
   81484:	52800022 	mov	w2, #0x1                   	// #1
    return ((gicd->TYPER & BIT_MASK(GICD_TYPER_ITLINENUM_OFF, GICD_TYPER_ITLINENUM_LEN) >>
   81488:	f9400421 	ldr	x1, [x1, #8]
   8148c:	b9400423 	ldr	w3, [x1, #4]
    gicd->CTLR = GICD_CTLR_EN_BIT;
   81490:	b9000022 	str	w2, [x1]
    gicc->PMR = -1;
   81494:	f940cc00 	ldr	x0, [x0, #408]
   81498:	12800002 	mov	w2, #0xffffffff            	// #-1
    gicc->CTLR = GICC_CTLR_EN_BIT;
   8149c:	52800021 	mov	w1, #0x1                   	// #1
    gicc->PMR = -1;
   814a0:	b9000402 	str	w2, [x0, #4]
    gicc->CTLR = GICC_CTLR_EN_BIT;
   814a4:	b9000001 	str	w1, [x0]
        gicd_init();
    }
    gicc_init();
}
...
00000000000814b0 <gic_set_enable>:
   ...
   814b0:	b00000a3 	adrp	x3, 96000 <JIS_state_table+0xb0>
    uint64_t bit = (1UL << int_id%(sizeof(uint32_t)*8));
   814b4:	12001004 	and	w4, w0, #0x1f
    uint64_t reg_ind = int_id/(sizeof(uint32_t)*8);
   814b8:	d345fc00 	lsr	x0, x0, #5
    uint64_t bit = (1UL << int_id%(sizeof(uint32_t)*8));
   814bc:	d2800022 	mov	x2, #0x1                   	// #1
        gicd->ISENABLER[reg_ind] = bit;
   814c0:	f940d063 	ldr	x3, [x3, #416]
    uint64_t bit = (1UL << int_id%(sizeof(uint32_t)*8));
   814c4:	9ac42042 	lsl	x2, x2, x4
        gicd->ISENABLER[reg_ind] = bit;
   814c8:	8b000860 	add	x0, x3, x0, lsl #2
    if(en)
   814cc:	72001c3f 	tst	w1, #0xff
   814d0:	54000060 	b.eq	814dc <gic_set_enable+0x2c>  // b.none
        gicd->ISENABLER[reg_ind] = bit;
   814d4:	b9010002 	str	w2, [x0, #256]
    else
   ...
   814d8:	d65f03c0 	ret
        gicd->ICENABLER[reg_ind] = bit;
   814dc:	b9018002 	str	w2, [x0, #384]
}
   814e0:	d65f03c0 	ret

00000000000814e4 <gic_set_trgt>:
   ...
   814e4:	b00000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
    uint32_t mask = ((1U << GIC_TARGET_BITS) - 1) << off;
   814e8:	927ee804 	and	x4, x0, #0x1ffffffffffffffc
{
   814ec:	12001c21 	and	w1, w1, #0xff
    uint64_t off = (int_id * GIC_TARGET_BITS) % (sizeof(uint32_t) * 8);
   814f0:	d37d0400 	ubfiz	x0, x0, #3, #2
    gicd->ITARGETSR[reg_ind] =
   814f4:	f940d042 	ldr	x2, [x2, #416]
    uint32_t mask = ((1U << GIC_TARGET_BITS) - 1) << off;
   814f8:	52801fe3 	mov	w3, #0xff                  	// #255
   814fc:	1ac02063 	lsl	w3, w3, w0
   81500:	8b040042 	add	x2, x2, x4
        (gicd->ITARGETSR[reg_ind] & ~mask) | ((trgt << off) & mask);
   81504:	1ac02021 	lsl	w1, w1, w0
   81508:	b9480040 	ldr	w0, [x2, #2048]
   8150c:	4a000021 	eor	w1, w1, w0
   81510:	0a030021 	and	w1, w1, w3
   81514:	4a000021 	eor	w1, w1, w0
    gicd->ITARGETSR[reg_ind] =
   81518:	b9080041 	str	w1, [x2, #2048]
}
   8151c:	d65f03c0 	ret

0000000000081520 <gic_get_trgt>:
{
    uint64_t reg_ind = (int_id * GIC_TARGET_BITS) / (sizeof(uint32_t) * 8);
    uint64_t off = (int_id * GIC_TARGET_BITS) % (sizeof(uint32_t) * 8);
    uint32_t mask = ((1U << GIC_TARGET_BITS) - 1) << off;
   ...
   81520:	b00000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
   81524:	927ee803 	and	x3, x0, #0x1ffffffffffffffc
    uint64_t off = (int_id * GIC_TARGET_BITS) % (sizeof(uint32_t) * 8);
   81528:	d37d0400 	ubfiz	x0, x0, #3, #2
    uint32_t mask = ((1U << GIC_TARGET_BITS) - 1) << off;
   8152c:	52801fe1 	mov	w1, #0xff                  	// #255
    return (gicd->ITARGETSR[reg_ind] & mask) >> off;
   81530:	f940d042 	ldr	x2, [x2, #416]
    uint32_t mask = ((1U << GIC_TARGET_BITS) - 1) << off;
   81534:	1ac02021 	lsl	w1, w1, w0
    return (gicd->ITARGETSR[reg_ind] & mask) >> off;
   81538:	8b030042 	add	x2, x2, x3
   8153c:	b9480042 	ldr	w2, [x2, #2048]
   81540:	0a020021 	and	w1, w1, w2
}
   81544:	1ac02420 	lsr	w0, w1, w0
   81548:	d65f03c0 	ret
   8154c:	d503201f 	nop

0000000000081550 <gic_send_sgi>:

void gic_send_sgi(uint64_t cpu_target, uint64_t sgi_num){
    gicd->SGIR   = (1UL << (GICD_SGIR_CPUTRGLST_OFF + cpu_target))
   81550:	b00000a3 	adrp	x3, 96000 <JIS_state_table+0xb0>
   81554:	11004002 	add	w2, w0, #0x10
        | (sgi_num & GICD_SGIR_SGIINTID_MSK);
   81558:	12000c21 	and	w1, w1, #0xf
    gicd->SGIR   = (1UL << (GICD_SGIR_CPUTRGLST_OFF + cpu_target))
   8155c:	d2800020 	mov	x0, #0x1                   	// #1
   81560:	f940d063 	ldr	x3, [x3, #416]
   81564:	9ac22000 	lsl	x0, x0, x2
        | (sgi_num & GICD_SGIR_SGIINTID_MSK);
   81568:	2a000021 	orr	w1, w1, w0
    gicd->SGIR   = (1UL << (GICD_SGIR_CPUTRGLST_OFF + cpu_target))
   8156c:	b90f0061 	str	w1, [x3, #3840]
}
   81570:	d65f03c0 	ret

0000000000081574 <gic_set_prio>:
void gic_set_prio(uint64_t int_id, uint8_t prio){
   ...
   81574:	b00000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
    uint64_t mask = ((1 << GIC_PRIO_BITS)-1) << off;
   81578:	927ee804 	and	x4, x0, #0x1ffffffffffffffc
void gic_set_prio(uint64_t int_id, uint8_t prio){
   8157c:	12001c21 	and	w1, w1, #0xff
    uint64_t off = (int_id*GIC_PRIO_BITS)%((sizeof(uint32_t)*8));
   81580:	d37d0400 	ubfiz	x0, x0, #3, #2
    gicd->IPRIORITYR[reg_ind] = (gicd->IPRIORITYR[reg_ind] & ~mask) | 
   81584:	f940d042 	ldr	x2, [x2, #416]
    uint64_t mask = ((1 << GIC_PRIO_BITS)-1) << off;
   81588:	52801fe3 	mov	w3, #0xff                  	// #255
   8158c:	1ac02063 	lsl	w3, w3, w0
   81590:	8b040042 	add	x2, x2, x4
        ((prio << off) & mask);
   81594:	1ac02021 	lsl	w1, w1, w0
    gicd->IPRIORITYR[reg_ind] = (gicd->IPRIORITYR[reg_ind] & ~mask) | 
   81598:	b9440040 	ldr	w0, [x2, #1024]
   8159c:	4a000021 	eor	w1, w1, w0
   815a0:	0a030021 	and	w1, w1, w3
   815a4:	4a000021 	eor	w1, w1, w0
   815a8:	b9040041 	str	w1, [x2, #1024]
}
   815ac:	d65f03c0 	ret

00000000000815b0 <gic_is_pending>:
bool gic_is_pending(uint64_t int_id){
   ...
   815b0:	b00000a1 	adrp	x1, 96000 <JIS_state_table+0xb0>
   815b4:	f940d022 	ldr	x2, [x1, #416]
    uint64_t reg_ind = int_id/(sizeof(uint32_t)*8);
   815b8:	d345fc01 	lsr	x1, x0, #5
    uint64_t off = int_id%(sizeof(uint32_t)*8);
   815bc:	92401000 	and	x0, x0, #0x1f
    return ((1U << off) & gicd->ISPENDR[reg_ind]) != 0;
   815c0:	8b010841 	add	x1, x2, x1, lsl #2
   815c4:	b9420021 	ldr	w1, [x1, #512]
   815c8:	1ac02420 	lsr	w0, w1, w0
}
   815cc:	12000000 	and	w0, w0, #0x1
   815d0:	d65f03c0 	ret

00000000000815d4 <gic_set_pending>:
void gic_set_pending(uint64_t int_id, bool pending){
   ...
   815d4:	b00000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
    uint64_t mask = 1U << int_id % (sizeof(uint32_t) * 8);
   815d8:	52800023 	mov	w3, #0x1                   	// #1
        gicd->ISPENDR[reg_ind] = mask;
   815dc:	f940d044 	ldr	x4, [x2, #416]
    uint64_t reg_ind = int_id / (sizeof(uint32_t) * 8);
   815e0:	d345fc02 	lsr	x2, x0, #5
    uint64_t mask = 1U << int_id % (sizeof(uint32_t) * 8);
   815e4:	1ac02060 	lsl	w0, w3, w0
        gicd->ISPENDR[reg_ind] = mask;
   815e8:	8b020882 	add	x2, x4, x2, lsl #2
    if (pending) {
   815ec:	72001c3f 	tst	w1, #0xff
   815f0:	54000060 	b.eq	815fc <gic_set_pending+0x28>  // b.none
        gicd->ISPENDR[reg_ind] = mask;
   815f4:	b9020040 	str	w0, [x2, #512]
    } else {
   ...
   815f8:	d65f03c0 	ret
        gicd->ICPENDR[reg_ind] = mask;
   815fc:	b9028040 	str	w0, [x2, #640]
}
   81600:	d65f03c0 	ret

0000000000081604 <gic_is_active>:
bool gic_is_active(uint64_t int_id){
   ...
   81604:	b00000a1 	adrp	x1, 96000 <JIS_state_table+0xb0>
   81608:	f940d022 	ldr	x2, [x1, #416]
    uint64_t reg_ind = int_id/(sizeof(uint32_t)*8);
   8160c:	d345fc01 	lsr	x1, x0, #5
    uint64_t off = int_id%(sizeof(uint32_t)*8);
   81610:	92401000 	and	x0, x0, #0x1f
    return ((1U << off) & gicd->ISACTIVER[reg_ind]) != 0;
   81614:	8b010841 	add	x1, x2, x1, lsl #2
   81618:	b9430021 	ldr	w1, [x1, #768]
   8161c:	1ac02420 	lsr	w0, w1, w0
}
   81620:	12000000 	and	w0, w0, #0x1
   81624:	d65f03c0 	ret
   81628:	d503201f 	nop
   8162c:	d503201f 	nop

0000000000081630 <gic_handle>:

void gic_handle(){
   81630:	a9be7bfd 	stp	x29, x30, [sp, #-32]!
   81634:	910003fd 	mov	x29, sp
   81638:	a90153f3 	stp	x19, x20, [sp, #16]

    uint64_t ack = gicc->IAR;
   8163c:	b00000b4 	adrp	x20, 96000 <JIS_state_table+0xb0>
   81640:	f940ce80 	ldr	x0, [x20, #408]
   81644:	b9400c13 	ldr	w19, [x0, #12]
    uint64_t id = ack & GICC_IAR_ID_MSK;
   81648:	12002660 	and	w0, w19, #0x3ff
    uint64_t src = (ack & GICC_IAR_CPU_MSK) >> GICC_IAR_CPU_OFF;

    if(id >= 1022) return;
   8164c:	710ff41f 	cmp	w0, #0x3fd
   81650:	54000088 	b.hi	81660 <gic_handle+0x30>  // b.pmore

    irq_handle(id);
   81654:	97fffd9b 	bl	80cc0 <irq_handle>
        
    gicc->EOIR = ack;
   81658:	f940ce80 	ldr	x0, [x20, #408]
   8165c:	b9001013 	str	w19, [x0, #16]
    
}
   81660:	a94153f3 	ldp	x19, x20, [sp, #16]
   81664:	a8c27bfd 	ldp	x29, x30, [sp], #32
   81668:	d65f03c0 	ret
	...
...
curr_el_spx_irq:       
   ...
   81aac:	a90e7bfd 	stp	x29, x30, [sp, #224]
    bl	gic_handle
   81ab0:	97fffee0 	bl	81630 <gic_handle>
    RESTORE_REGS
...
curr_el_spx_fiq:         
   ...
   81b2c:	a90e7bfd 	stp	x29, x30, [sp, #224]
    bl	gic_handle
   81b30:	97fffec0 	bl	81630 <gic_handle>
    RESTORE_REGS
...
0000000000082000 <__errno>:
   82000:	900000a0 	adrp	x0, 96000 <JIS_state_table+0xb0>
   82004:	f940e000 	ldr	x0, [x0, #448]
   82008:	d65f03c0 	ret
...
0000000000082110 <putchar>:
   82110:	900000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
   82114:	2a0003e1 	mov	w1, w0
   82118:	f940e040 	ldr	x0, [x2, #448]
...
0000000000082130 <_puts_r>:
   ...
   82158:	f0000081 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8215c:	d2800023 	mov	x3, #0x1                   	// #1
   82160:	91160021 	add	x1, x1, #0x580
...
0000000000082220 <puts>:
   82220:	900000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
   82224:	aa0003e1 	mov	x1, x0
   82228:	f940e040 	ldr	x0, [x2, #448]
   8222c:	17ffffc1 	b	82130 <_puts_r>

0000000000082230 <stdio_exit_handler>:
   82230:	900000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
   82234:	d0000041 	adrp	x1, 8c000 <currentlocale+0x80>
   82238:	910c8042 	add	x2, x2, #0x320
   8223c:	9131c021 	add	x1, x1, #0xc70
   82240:	900000a0 	adrp	x0, 96000 <JIS_state_table+0xb0>
   82244:	91072000 	add	x0, x0, #0x1c8
...
00000000000824b0 <__sfp>:
   ...
   824e4:	900000b4 	adrp	x20, 96000 <JIS_state_table+0xb0>
   824e8:	910c8294 	add	x20, x20, #0x320
...
0000000000082690 <__fp_lock_all>:
   ...
   826a8:	900000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
   826ac:	90000001 	adrp	x1, 82000 <__errno>
   826b0:	910c8042 	add	x2, x2, #0x320
...
00000000000826c0 <__fp_unlock_all>:
   ...
   826c4:	900000a2 	adrp	x2, 96000 <JIS_state_table+0xb0>
   826c8:	90000001 	adrp	x1, 82000 <__errno>
   826cc:	910003fd 	mov	x29, sp
   826d0:	910c8042 	add	x2, x2, #0x320
...
0000000000082820 <snprintf>:
   ...
   82824:	900000a9 	adrp	x9, 96000 <JIS_state_table+0xb0>
...
00000000000830c0 <_write_r>:
   ...
   830e4:	97fff717 	bl	80d40 <_write>
...
00000000000831c0 <_svfprintf_r>:
   ...
   83254:	f0000094 	adrp	x20, 96000 <JIS_state_table+0xb0>
   83258:	aa1603fc 	mov	x28, x22
   8325c:	91314294 	add	x20, x20, #0xc50
   83260:	aa0903f9 	mov	x25, x9
   83264:	d0000080 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   83268:	91179000 	add	x0, x0, #0x5e4
   ...
   834f8:	d0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   834fc:	b9416be1 	ldr	w1, [sp, #360]
   83500:	911a8084 	add	x4, x4, #0x6a0
   ...
   83638:	d0000082 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   8363c:	d2800024 	mov	x4, #0x1                   	// #1
   83640:	91178042 	add	x2, x2, #0x5e0
   ...
   836a8:	d0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   836ac:	b9416be1 	ldr	w1, [sp, #360]
   836b0:	911a8084 	add	x4, x4, #0x6a0
   ...
   83730:	d000008d 	adrp	x13, 95000 <pmu_event_descr+0xa0>
   83734:	b9416be1 	ldr	w1, [sp, #360]
   83738:	911ac1ad 	add	x13, x13, #0x6b0
   ...
   83884:	d0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   83888:	d2800027 	mov	x7, #0x1                   	// #1
   8388c:	91178084 	add	x4, x4, #0x5e0
   ...
   83964:	d0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   83968:	b9416be1 	ldr	w1, [sp, #360]
   8396c:	911a8084 	add	x4, x4, #0x6a0
   ...
   83c44:	d000008d 	adrp	x13, 95000 <pmu_event_descr+0xa0>
   83c48:	b9416be1 	ldr	w1, [sp, #360]
   83c4c:	911ac1ad 	add	x13, x13, #0x6b0
   ...
   83d9c:	d0000080 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   83da0:	d0000085 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   83da4:	7101211f 	cmp	w8, #0x48
   83da8:	91164000 	add	x0, x0, #0x590
   83dac:	911620a5 	add	x5, x5, #0x588
   ...
   83ef0:	d0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   83ef4:	b9416be1 	ldr	w1, [sp, #360]
   83ef8:	911a8084 	add	x4, x4, #0x6a0
   ...
   83fdc:	d0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   83fe0:	911a8084 	add	x4, x4, #0x6a0
   ...
   843a0:	b0000083 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   843a4:	321f013a 	orr	w26, w9, #0x2
   843a8:	9116a063 	add	x3, x3, #0x5a8
   ...
   84878:	b0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   8487c:	b9416be1 	ldr	w1, [sp, #360]
   84880:	911a8084 	add	x4, x4, #0x6a0
   ...
   8491c:	b0000080 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   84920:	91170000 	add	x0, x0, #0x5c0
   ...
   84964:	b0000080 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   84968:	2a1703e9 	mov	w9, w23
   8496c:	9116a000 	add	x0, x0, #0x5a8
   ...
   849ac:	b0000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   849b0:	f94083fc 	ldr	x28, [sp, #256]
   849b4:	911a8084 	add	x4, x4, #0x6a0
   ...
   84b84:	b0000089 	adrp	x9, 95000 <pmu_event_descr+0xa0>
   84b88:	b9416be2 	ldr	w2, [sp, #360]
   84b8c:	911a8129 	add	x9, x9, #0x6a0
   ...
   84cac:	b0000082 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   84cb0:	aa1603e1 	mov	x1, x22
   84cb4:	911a8044 	add	x4, x2, #0x6a0
   ...
   84d58:	b0000080 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   84d5c:	b0000082 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   84d60:	91170000 	add	x0, x0, #0x5c0
   84d64:	9116a042 	add	x2, x2, #0x5a8
   ...
   85080:	90000083 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   85084:	39400322 	ldrb	w2, [x25]
   85088:	aa1603e1 	mov	x1, x22
   8508c:	911a8064 	add	x4, x3, #0x6a0
   ...
   850a0:	90000085 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   850a4:	2a0303fb 	mov	w27, w3
   850a8:	911760b8 	add	x24, x5, #0x5d8
   ...
   859a0:	90000083 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   859a4:	39400322 	ldrb	w2, [x25]
   859a8:	aa1603e1 	mov	x1, x22
   859ac:	911a8064 	add	x4, x3, #0x6a0
   ...
   85af4:	90000084 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   85af8:	4b0203fa 	neg	w26, w2
   85afc:	911a8084 	add	x4, x4, #0x6a0
   ...
   85e70:	90000080 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   85e74:	90000085 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   85e78:	7101211f 	cmp	w8, #0x48
   85e7c:	91168000 	add	x0, x0, #0x5a0
   85e80:	911660a5 	add	x5, x5, #0x598
...
00000000000862f0 <_vfprintf_r>:
   ...
   863c0:	90000094 	adrp	x20, 96000 <JIS_state_table+0xb0>
   863c4:	91314294 	add	x20, x20, #0xc50
   863c8:	a9046bf9 	stp	x25, x26, [sp, #64]
   863cc:	aa0903f9 	mov	x25, x9
   863d0:	f0000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   863d4:	a90573fb 	stp	x27, x28, [sp, #80]
   863d8:	aa1603fc 	mov	x28, x22
   863dc:	911b0000 	add	x0, x0, #0x6c0
   ...
   866f0:	f0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   866f4:	b9418be1 	ldr	w1, [sp, #392]
   866f8:	911e0084 	add	x4, x4, #0x780
   ...
   86830:	f0000062 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   86834:	d2800024 	mov	x4, #0x1                   	// #1
   86838:	91178042 	add	x2, x2, #0x5e0
   ...
   868a0:	f0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   868a4:	b9418be1 	ldr	w1, [sp, #392]
   868a8:	911e0084 	add	x4, x4, #0x780
   ...
   86928:	f000006d 	adrp	x13, 95000 <pmu_event_descr+0xa0>
   8692c:	b9418be1 	ldr	w1, [sp, #392]
   86930:	911e41ad 	add	x13, x13, #0x790
   ...
   86a38:	f0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   86a3c:	d2800027 	mov	x7, #0x1                   	// #1
   86a40:	91178084 	add	x4, x4, #0x5e0
   ...
   86b70:	f0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   86b74:	b9418be1 	ldr	w1, [sp, #392]
   86b78:	911e0084 	add	x4, x4, #0x780
   ...
   86eac:	f0000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   86eb0:	f0000065 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   86eb4:	7101211f 	cmp	w8, #0x48
   86eb8:	91164000 	add	x0, x0, #0x590
   86ebc:	911620a5 	add	x5, x5, #0x588
   ...
   86f54:	f0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   86f58:	911e0084 	add	x4, x4, #0x780
   ...
   87330:	d0000063 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   87334:	321f013a 	orr	w26, w9, #0x2
   87338:	9116a063 	add	x3, x3, #0x5a8
   ...
   874bc:	d000006d 	adrp	x13, 95000 <pmu_event_descr+0xa0>
   874c0:	b9418be1 	ldr	w1, [sp, #392]
   874c4:	911e41ad 	add	x13, x13, #0x790
   ...
   876d8:	d0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   876dc:	b9418be1 	ldr	w1, [sp, #392]
   876e0:	911e0084 	add	x4, x4, #0x780
   ...
   87a50:	d0000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   87a54:	91170000 	add	x0, x0, #0x5c0
   ...
   87a88:	d0000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   87a8c:	2a1703e9 	mov	w9, w23
   87a90:	9116a000 	add	x0, x0, #0x5a8
   ...
   87d24:	d0000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   87d28:	d0000062 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   87d2c:	91170000 	add	x0, x0, #0x5c0
   87d30:	9116a042 	add	x2, x2, #0x5a8
   ...
   88074:	b0000065 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   88078:	2a0303fb 	mov	w27, w3
   8807c:	911760b8 	add	x24, x5, #0x5d8
   ...
   880d8:	b0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   880dc:	b9418be1 	ldr	w1, [sp, #392]
   880e0:	911e0084 	add	x4, x4, #0x780
   ...
   887d8:	b0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   887dc:	f94083fc 	ldr	x28, [sp, #256]
   887e0:	911e0084 	add	x4, x4, #0x780
   ...
   888e0:	b0000069 	adrp	x9, 95000 <pmu_event_descr+0xa0>
   888e4:	b9418be2 	ldr	w2, [sp, #392]
   888e8:	911e0129 	add	x9, x9, #0x780
   ...
   88948:	b0000063 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   8894c:	b9418be2 	ldr	w2, [sp, #392]
   88950:	aa1603e1 	mov	x1, x22
   88954:	911e0064 	add	x4, x3, #0x780
   ...
   88974:	b0000062 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   88978:	aa1603e1 	mov	x1, x22
   8897c:	911e0044 	add	x4, x2, #0x780
   ...
   889d4:	b0000063 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   889d8:	39400322 	ldrb	w2, [x25]
   889dc:	aa1603e1 	mov	x1, x22
   889e0:	911e0064 	add	x4, x3, #0x780
   ...
   88c68:	b0000063 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   88c6c:	39400322 	ldrb	w2, [x25]
   88c70:	aa1603e1 	mov	x1, x22
   88c74:	911e0064 	add	x4, x3, #0x780
   ...
   88da4:	b0000064 	adrp	x4, 95000 <pmu_event_descr+0xa0>
   88da8:	4b0203fa 	neg	w26, w2
   88dac:	911e0084 	add	x4, x4, #0x780
   ...
   89110:	90000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   89114:	90000065 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   89118:	7101211f 	cmp	w8, #0x48
   8911c:	91168000 	add	x0, x0, #0x5a0
   89120:	911660a5 	add	x5, x5, #0x598
...
0000000000089840 <_vfiprintf_r>:
   ...
   898e0:	b0000075 	adrp	x21, 96000 <JIS_state_table+0xb0>
   898e4:	913142b5 	add	x21, x21, #0xc50
   898e8:	a90573fb 	stp	x27, x28, [sp, #80]
   898ec:	aa1703fb 	mov	x27, x23
   898f0:	90000060 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   898f4:	911e8000 	add	x0, x0, #0x7a0
   ...
   89d20:	9000006a 	adrp	x10, 95000 <pmu_event_descr+0xa0>
   89d24:	9121814a 	add	x10, x10, #0x860
   ...
   89e50:	9000006a 	adrp	x10, 95000 <pmu_event_descr+0xa0>
   89e54:	9121814a 	add	x10, x10, #0x860
   ...
   8a008:	f000004b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   8a00c:	9121c16b 	add	x11, x11, #0x870
   ...
   8a13c:	f000004b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   8a140:	9121c16b 	add	x11, x11, #0x870
   ...
   8a638:	f0000042 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   8a63c:	321f0284 	orr	w4, w20, #0x2
   8a640:	9116a042 	add	x2, x2, #0x5a8
   ...
   8a91c:	f0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8a920:	91170021 	add	x1, x1, #0x5c0
   ...
   8a960:	f0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8a964:	2a1903e8 	mov	w8, w25
   8a968:	9116a021 	add	x1, x1, #0x5a8
   ...
   8ac40:	f0000047 	adrp	x7, 95000 <pmu_event_descr+0xa0>
   8ac44:	f94047f8 	ldr	x24, [sp, #136]
   8ac48:	2a1903e4 	mov	w4, w25
   8ac4c:	911760fc 	add	x28, x7, #0x5d8
   ...
   8b040:	d000004b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   8b044:	2a0203ef 	mov	w15, w2
   8b048:	9121c16b 	add	x11, x11, #0x870
   8b04c:	17fffc24 	b	8a0dc <_vfiprintf_r+0x89c>
   8b050:	d000004b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   8b054:	11000446 	add	w6, w2, #0x1
   8b058:	9121c16b 	add	x11, x11, #0x870
   ...
   8b08c:	d000004a 	adrp	x10, 95000 <pmu_event_descr+0xa0>
   8b090:	2a0b03ed 	mov	w13, w11
   8b094:	9121814a 	add	x10, x10, #0x860
...
000000000008b580 <_malloc_r>:
   ...
   8b5bc:	f0000054 	adrp	x20, 96000 <JIS_state_table+0xb0>
   8b5c0:	910d4294 	add	x20, x20, #0x350
   ...
   8b5e0:	f0000047 	adrp	x7, 96000 <JIS_state_table+0xb0>
   8b5e4:	910d80e7 	add	x7, x7, #0x360
   ...
   8b77c:	f0000054 	adrp	x20, 96000 <JIS_state_table+0xb0>
   8b780:	910d4294 	add	x20, x20, #0x350
   ...
   8b7c8:	f0000047 	adrp	x7, 96000 <JIS_state_table+0xb0>
   8b7cc:	910d80e7 	add	x7, x7, #0x360
   ...
   8b81c:	f000005c 	adrp	x28, 96000 <JIS_state_table+0xb0>
   8b820:	f9478c21 	ldr	x1, [x1, #3864]
   8b824:	d28203e3 	mov	x3, #0x101f                	// #4127
   8b828:	f9419f82 	ldr	x2, [x28, #824]
   ...
   8bc18:	d0000042 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   8bc1c:	d10062e1 	sub	x1, x23, #0x18
   8bc20:	3dc22040 	ldr	q0, [x2, #2176]
   ...
   8bd08:	f9019f98 	str	x24, [x28, #824]
...
000000000008bdb0 <_wcrtomb_r>:
   ...
   8bdc8:	f0000043 	adrp	x3, 96000 <JIS_state_table+0xb0>
   8bdcc:	9a930093 	csel	x19, x4, x19, eq	// eq = none
   8bdd0:	aa0003f4 	mov	x20, x0
   8bdd4:	f9469864 	ldr	x4, [x3, #3376]
...
000000000008be40 <wcrtomb>:
   ...
   8be44:	f0000044 	adrp	x4, 96000 <JIS_state_table+0xb0>
   8be48:	f0000043 	adrp	x3, 96000 <JIS_state_table+0xb0>
   8be4c:	910003fd 	mov	x29, sp
   8be50:	a90153f3 	stp	x19, x20, [sp, #16]
   8be54:	f100005f 	cmp	x2, #0x0
   8be58:	f940e094 	ldr	x20, [x4, #448]
   8be5c:	9104f284 	add	x4, x20, #0x13c
   8be60:	9a820093 	csel	x19, x4, x2, eq	// eq = none
   8be64:	f9469864 	ldr	x4, [x3, #3376]
...
000000000008bf80 <currentlocale>:
   ...
   8bf8c:	f0000054 	adrp	x20, 96000 <JIS_state_table+0xb0>
   8bf90:	91314294 	add	x20, x20, #0xc50
   8bf94:	a9025bf5 	stp	x21, x22, [sp, #32]
   8bf98:	f0000055 	adrp	x21, 96000 <JIS_state_table+0xb0>
   8bf9c:	913242b5 	add	x21, x21, #0xc90
   8bfa0:	f9001bf7 	str	x23, [sp, #48]
   8bfa4:	f0000057 	adrp	x23, 96000 <JIS_state_table+0xb0>
   8bfa8:	912d82f7 	add	x23, x23, #0xb60
   8bfac:	f0000056 	adrp	x22, 96000 <JIS_state_table+0xb0>
   8bfb0:	aa1503f3 	mov	x19, x21
   8bfb4:	9131c2c1 	add	x1, x22, #0xc70
   8bfb8:	91038294 	add	x20, x20, #0xe0
   8bfbc:	9131c2d6 	add	x22, x22, #0xc70
   ...
   8bff8:	d65f03c0 	ret
   8bffc:	d0000053 	adrp	x19, 95000 <pmu_event_descr+0xa0>
   8c000:	91224273 	add	x19, x19, #0x890
   8c004:	d503201f 	nop
...
000000000008c050 <__loadlocale>:
   ...
   8c0a4:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c0a8:	b0000055 	adrp	x21, 95000 <pmu_event_descr+0xa0>
   8c0ac:	91226021 	add	x1, x1, #0x898
   8c0b0:	912282b5 	add	x21, x21, #0x8a0
   ...
   8c13c:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c140:	aa1503e0 	mov	x0, x21
   8c144:	9122c021 	add	x1, x1, #0x8b0
   ...
   8c178:	b0000040 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8c17c:	9126c000 	add	x0, x0, #0x9b0
   ...
   8c1b0:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c1b4:	91250021 	add	x1, x1, #0x940
   ...
   8c234:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c238:	aa1503e0 	mov	x0, x21
   8c23c:	9122a021 	add	x1, x1, #0x8a8
   ...
   8c2ec:	b000005b 	adrp	x27, 95000 <pmu_event_descr+0xa0>
   8c2f0:	9123a37b 	add	x27, x27, #0x8e8
   ...
   8c304:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c308:	aa1503e0 	mov	x0, x21
   8c30c:	9123c021 	add	x1, x1, #0x8f0
   ...
   8c380:	d65f03c0 	ret
   8c384:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c388:	aa1503e0 	mov	x0, x21
   8c38c:	91262021 	add	x1, x1, #0x988
   8c390:	94001aec 	bl	92f40 <strcasecmp>
   8c394:	35fff460 	cbnz	w0, 8c220 <__loadlocale+0x1d0>
   8c398:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c39c:	aa1503e0 	mov	x0, x21
   8c3a0:	91264021 	add	x1, x1, #0x990
   ...
   8c3c0:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c3c4:	aa1503e0 	mov	x0, x21
   8c3c8:	91252021 	add	x1, x1, #0x948
   ...
   8c40c:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c410:	91258021 	add	x1, x1, #0x960
   8c414:	94000f5b 	bl	90180 <strcpy>
   8c418:	17ffffe4 	b	8c3a8 <__loadlocale+0x358>
   8c41c:	b000005b 	adrp	x27, 95000 <pmu_event_descr+0xa0>
   8c420:	9123e37b 	add	x27, x27, #0x8f8
   ...
   8c458:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c45c:	aa1503e0 	mov	x0, x21
   8c460:	91248021 	add	x1, x1, #0x920
   ...
   8c474:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c478:	d2800082 	mov	x2, #0x4                   	// #4
   8c47c:	9124a021 	add	x1, x1, #0x928
   ...
   8c4dc:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c4e0:	9124c021 	add	x1, x1, #0x930
   ...
   8c52c:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c530:	aa1503e0 	mov	x0, x21
   8c534:	91266021 	add	x1, x1, #0x998
   ...
   8c548:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c54c:	91268021 	add	x1, x1, #0x9a0
   ...
   8c56c:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c570:	9126a021 	add	x1, x1, #0x9a8
   8c574:	94000f03 	bl	90180 <strcpy>
   8c578:	17ffff8c 	b	8c3a8 <__loadlocale+0x358>
   8c57c:	b000005b 	adrp	x27, 95000 <pmu_event_descr+0xa0>
   8c580:	9124637b 	add	x27, x27, #0x918
   ...
   8c5b8:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c5bc:	aa1503e0 	mov	x0, x21
   8c5c0:	9125a021 	add	x1, x1, #0x968
   ...
   8c5d4:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c5d8:	9125e021 	add	x1, x1, #0x978
   ...
   8c5f8:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c5fc:	91260021 	add	x1, x1, #0x980
   8c600:	94000ee0 	bl	90180 <strcpy>
   8c604:	17ffff69 	b	8c3a8 <__loadlocale+0x358>
   8c608:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c60c:	aa1503e0 	mov	x0, x21
   8c610:	91240021 	add	x1, x1, #0x900
   ...
   8c624:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c628:	91242021 	add	x1, x1, #0x908
   ...
   8c648:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c64c:	91244021 	add	x1, x1, #0x910
   ...
   8c66c:	b000005b 	adrp	x27, 95000 <pmu_event_descr+0xa0>
   8c670:	9122a37b 	add	x27, x27, #0x8a8
   ...
   8c708:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c70c:	52800018 	mov	w24, #0x0                   	// #0
   8c710:	91230021 	add	x1, x1, #0x8c0
   ...
   8c730:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c734:	5280001a 	mov	w26, #0x0                   	// #0
   8c738:	91234021 	add	x1, x1, #0x8d0
   ...
   8c750:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c754:	91238021 	add	x1, x1, #0x8e0
   ...
   8c7b0:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c7b4:	91254021 	add	x1, x1, #0x950
   8c7b8:	94000e72 	bl	90180 <strcpy>
   8c7bc:	17fffefb 	b	8c3a8 <__loadlocale+0x358>
   8c7c0:	aa1503e0 	mov	x0, x21
   8c7c4:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c7c8:	91256021 	add	x1, x1, #0x958
...
000000000008c7e0 <__get_locale_env>:
   ...
   8c7f4:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c7f8:	91288021 	add	x1, x1, #0xa20
   ...
   8c80c:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c810:	91310021 	add	x1, x1, #0xc40
   ...
   8c82c:	b0000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8c830:	aa1303e0 	mov	x0, x19
   8c834:	9128a021 	add	x1, x1, #0xa28
   ...
   8c848:	d0000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8c84c:	91380000 	add	x0, x0, #0xe00
...
000000000008c860 <_setlocale_r>:
   ...
   8c898:	d0000055 	adrp	x21, 96000 <JIS_state_table+0xb0>
   8c89c:	910182f7 	add	x23, x23, #0x60
   8c8a0:	9131c2b5 	add	x21, x21, #0xc70
   ...
   8c92c:	d0000059 	adrp	x25, 96000 <JIS_state_table+0xb0>
   8c930:	aa1a03f6 	mov	x22, x26
   8c934:	aa1703f4 	mov	x20, x23
   8c938:	91314339 	add	x25, x25, #0xc50
   ...
   8c9c4:	d0000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8c9c8:	91314000 	add	x0, x0, #0xc50
   ...
   8cabc:	d0000041 	adrp	x1, 96000 <JIS_state_table+0xb0>
   8cac0:	91314021 	add	x1, x1, #0xc50
   8cac4:	7100037f 	cmp	w27, #0x0
   8cac8:	8b010000 	add	x0, x0, x1
   8cacc:	d0000053 	adrp	x19, 96000 <JIS_state_table+0xb0>
   8cad0:	912d8273 	add	x19, x19, #0xb60
   ...
   8cb2c:	b0000040 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8cb30:	b9400315 	ldr	w21, [x24]
   8cb34:	91228016 	add	x22, x0, #0x8a0
...
000000000008cc10 <__locale_mb_cur_max>:
   8cc10:	d0000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8cc14:	3976c000 	ldrb	w0, [x0, #3504]
   8cc18:	d65f03c0 	ret
   8cc1c:	00000000 	udf	#0

000000000008cc20 <setlocale>:
   8cc20:	d0000043 	adrp	x3, 96000 <JIS_state_table+0xb0>
   8cc24:	aa0103e2 	mov	x2, x1
   8cc28:	2a0003e1 	mov	w1, w0
   8cc2c:	f940e060 	ldr	x0, [x3, #448]
...
000000000008cc50 <_localeconv_r>:
   8cc50:	d0000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8cc54:	91354000 	add	x0, x0, #0xd50
   8cc58:	d65f03c0 	ret
   8cc5c:	00000000 	udf	#0

000000000008cc60 <localeconv>:
   8cc60:	d0000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8cc64:	91354000 	add	x0, x0, #0xd50
   8cc68:	d65f03c0 	ret
...
000000000008cdb0 <fclose>:
   8cdb0:	d0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8cdb4:	aa0003e1 	mov	x1, x0
   8cdb8:	f940e040 	ldr	x0, [x2, #448]
...
000000000008d4d0 <putc>:
   ...
   8d4d4:	b0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8d4d8:	910003fd 	mov	x29, sp
   8d4dc:	f90013f5 	str	x21, [sp, #32]
   8d4e0:	f940e055 	ldr	x21, [x2, #448]
...
000000000008d7d0 <wcsrtombs>:
   8d7d0:	b0000046 	adrp	x6, 96000 <JIS_state_table+0xb0>
   8d7d4:	aa0003e4 	mov	x4, x0
   8d7d8:	aa0103e5 	mov	x5, x1
   8d7dc:	aa0403e1 	mov	x1, x4
   8d7e0:	f940e0c0 	ldr	x0, [x6, #448]
...
000000000008d9b0 <_dtoa_r>:
   ...
   8da68:	90000040 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8da6c:	91178400 	add	x0, x0, #0x5e1
   8da70:	f9000300 	str	x0, [x24]
   8da74:	90000055 	adrp	x21, 95000 <pmu_event_descr+0xa0>
   8da78:	911782b5 	add	x21, x21, #0x5e0
   ...
   8db1c:	90000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8db20:	1e620003 	scvtf	d3, w0
   8db24:	1e613841 	fsub	d1, d2, d1
   8db28:	fd468424 	ldr	d4, [x1, #3336]
   8db2c:	90000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8db30:	fd468820 	ldr	d0, [x1, #3344]
   8db34:	90000041 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8db38:	1f440020 	fmadd	d0, d1, d4, d0
   8db3c:	fd468c22 	ldr	d2, [x1, #3352]
   ...
   8db60:	b0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8db64:	9102c044 	add	x4, x2, #0xb0
   ...
   8dc68:	b0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8dc6c:	aa0003e1 	mov	x1, x0
   8dc70:	9102c044 	add	x4, x2, #0xb0
   ...
   8dc8c:	b0000041 	adrp	x1, 96000 <JIS_state_table+0xb0>
   8dc90:	12000c00 	and	w0, w0, #0xf
   8dc94:	52800062 	mov	w2, #0x3                   	// #3
   8dc98:	fd405021 	ldr	d1, [x1, #160]
   8dc9c:	1e611921 	fdiv	d1, d9, d1
   8dca0:	34000160 	cbz	w0, 8dccc <_dtoa_r+0x31c>
   8dca4:	b0000041 	adrp	x1, 96000 <JIS_state_table+0xb0>
   8dca8:	91020021 	add	x1, x1, #0x80
   8dcac:	d503201f 	nop
   ...
   8dd18:	b0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8dd1c:	51000509 	sub	w9, w8, #0x1
   8dd20:	9102c044 	add	x4, x2, #0xb0
   ...
   8ddbc:	b0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8ddc0:	b94083e1 	ldr	w1, [sp, #128]
   8ddc4:	9102c044 	add	x4, x2, #0xb0
   ...
   8de28:	90000055 	adrp	x21, 95000 <pmu_event_descr+0xa0>
   8de2c:	b40050d8 	cbz	x24, 8e844 <_dtoa_r+0xe94>
   8de30:	90000040 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8de34:	9131e2b5 	add	x21, x21, #0xc78
   8de38:	91320000 	add	x0, x0, #0xc80
   ...
   8de64:	90000055 	adrp	x21, 95000 <pmu_event_descr+0xa0>
   8de68:	b4004f38 	cbz	x24, 8e84c <_dtoa_r+0xe9c>
   8de6c:	90000040 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8de70:	913222b5 	add	x21, x21, #0xc88
   8de74:	91322c00 	add	x0, x0, #0xc8b
   ...
   8df90:	b0000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8df94:	9102c044 	add	x4, x2, #0xb0
   ...
   8dfb4:	b0000044 	adrp	x4, 96000 <JIS_state_table+0xb0>
   8dfb8:	91020084 	add	x4, x4, #0x80
   ...
   8e844:	9131e2b5 	add	x21, x21, #0xc78
   8e848:	17fffc8d 	b	8da7c <_dtoa_r+0xcc>
   8e84c:	913222b5 	add	x21, x21, #0xc88
   ...
   8ea9c:	f0000023 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   8eaa0:	f0000020 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8eaa4:	91324063 	add	x3, x3, #0xc90
   8eaa8:	9132a000 	add	x0, x0, #0xca8
   ...
   8eb0c:	f0000023 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   8eb10:	f0000020 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   8eb14:	91324063 	add	x3, x3, #0xc90
   8eb18:	9132a000 	add	x0, x0, #0xca8
...
000000000008eb30 <__set_ctype>:
   8eb30:	f0000021 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   8eb34:	91348021 	add	x1, x1, #0xd20
...
000000000008eb40 <_close_r>:
   ...
   8eb5c:	97ffc8a1 	bl	80de0 <_close>
...
000000000008ef10 <fflush>:
   ...
   8ef24:	90000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8ef28:	f90013f5 	str	x21, [sp, #32]
   8ef2c:	f940e015 	ldr	x21, [x0, #448]
   ...
   8efac:	90000042 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8efb0:	90000001 	adrp	x1, 8e000 <_dtoa_r+0x650>
   8efb4:	90000040 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8efb8:	910c8042 	add	x2, x2, #0x320
   8efbc:	91394021 	add	x1, x1, #0xe50
   8efc0:	91072000 	add	x0, x0, #0x1c8
...
000000000008f130 <_realloc_r>:
   ...
   8f194:	f0000021 	adrp	x1, 96000 <JIS_state_table+0xb0>
   8f198:	a90573fb 	stp	x27, x28, [sp, #80]
   8f19c:	910d403c 	add	x28, x1, #0x350
...
000000000008f6a0 <_malloc_trim_r>:
   ...
   8f6ac:	f0000036 	adrp	x22, 96000 <JIS_state_table+0xb0>
   8f6b0:	910d42d6 	add	x22, x22, #0x350
...
000000000008f7c0 <_free_r>:
   ...
   8f7e4:	f0000020 	adrp	x0, 96000 <JIS_state_table+0xb0>
   8f7e8:	910d4000 	add	x0, x0, #0x350
   ...
   8f950:	f0000025 	adrp	x5, 96000 <JIS_state_table+0xb0>
   8f954:	8b010042 	add	x2, x2, x1
   8f958:	910d80a5 	add	x5, x5, #0x360
   ...
   8f9a4:	f0000022 	adrp	x2, 96000 <JIS_state_table+0xb0>
   8f9a8:	b2400024 	orr	x4, x1, #0x1
   8f9ac:	f9000464 	str	x4, [x3, #8]
   8f9b0:	f941a042 	ldr	x2, [x2, #832]
...
000000000008faa0 <_strtol_l.part.0>:
   8faa0:	d0000027 	adrp	x7, 95000 <pmu_event_descr+0xa0>
   8faa4:	aa0003ec 	mov	x12, x0
   8faa8:	aa0103e6 	mov	x6, x1
   8faac:	913484e7 	add	x7, x7, #0xd21
...
000000000008fc50 <strtol_l>:
   8fc50:	f0000024 	adrp	x4, 96000 <JIS_state_table+0xb0>
   8fc54:	7100905f 	cmp	w2, #0x24
   8fc58:	7a419844 	ccmp	w2, #0x1, #0x4, ls	// ls = plast
   8fc5c:	f940e084 	ldr	x4, [x4, #448]
...
000000000008fca0 <strtol>:
   8fca0:	f0000024 	adrp	x4, 96000 <JIS_state_table+0xb0>
   8fca4:	7100905f 	cmp	w2, #0x24
   8fca8:	7a419844 	ccmp	w2, #0x1, #0x4, ls	// ls = plast
   8fcac:	f940e084 	ldr	x4, [x4, #448]
...
000000000008fcf0 <strncasecmp>:
   ...
   8fcf8:	d0000027 	adrp	x7, 95000 <pmu_event_descr+0xa0>
   8fcfc:	d2800004 	mov	x4, #0x0                   	// #0
   8fd00:	913484e7 	add	x7, x7, #0xd21
...
000000000008fd70 <_findenv_r>:
   ...
   8fd7c:	f0000038 	adrp	x24, 96000 <JIS_state_table+0xb0>
   ...
   8fd98:	f9471314 	ldr	x20, [x24, #3616]
   ...
   8fe34:	d65f03c0 	ret
   8fe38:	f9471301 	ldr	x1, [x24, #3616]
...
000000000008ff20 <_fstat_r>:
   ...
   8ff40:	97ffc3ac 	bl	80df0 <_fstat>
...
000000000008ff70 <_isatty_r>:
   ...
   8ff8c:	97ffc39d 	bl	80e00 <_isatty>
...
000000000008ffc0 <_lseek_r>:
   ...
   8ffe4:	97ffc374 	bl	80db4 <_lseek>
...
00000000000906a0 <fputwc>:
   ...
   906a4:	d0000022 	adrp	x2, 96000 <JIS_state_table+0xb0>
   906a8:	910003fd 	mov	x29, sp
   906ac:	f90013f5 	str	x21, [sp, #32]
   906b0:	f940e055 	ldr	x21, [x2, #448]
...
0000000000090770 <_wctomb_r>:
   90770:	d0000024 	adrp	x4, 96000 <JIS_state_table+0xb0>
   90774:	f9469884 	ldr	x4, [x4, #3376]
...
0000000000090d00 <_svfiprintf_r>:
   ...
   90d5c:	d0000035 	adrp	x21, 96000 <JIS_state_table+0xb0>
   90d60:	aa1703fc 	mov	x28, x23
   90d64:	913142b5 	add	x21, x21, #0xc50
   90d68:	b0000020 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   90d6c:	91389000 	add	x0, x0, #0xe24
   ...
   91094:	9000002b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   91098:	913b816b 	add	x11, x11, #0xee0
   ...
   911b8:	9000002b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   911bc:	913b816b 	add	x11, x11, #0xee0
   ...
   9133c:	9000002a 	adrp	x10, 95000 <pmu_event_descr+0xa0>
   91340:	913bc14a 	add	x10, x10, #0xef0
   ...
   914a4:	9000002a 	adrp	x10, 95000 <pmu_event_descr+0xa0>
   914a8:	b940fbe0 	ldr	w0, [sp, #248]
   914ac:	913bc14a 	add	x10, x10, #0xef0
   ...
   91a34:	90000022 	adrp	x2, 95000 <pmu_event_descr+0xa0>
   91a38:	321f0284 	orr	w4, w20, #0x2
   91a3c:	9116a042 	add	x2, x2, #0x5a8
   ...
   91cf4:	90000021 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   91cf8:	91170021 	add	x1, x1, #0x5c0
   ...
   91d5c:	90000021 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   91d60:	9116a021 	add	x1, x1, #0x5a8
   ...
   91f20:	90000027 	adrp	x7, 95000 <pmu_event_descr+0xa0>
   91f24:	f94047f8 	ldr	x24, [sp, #136]
   91f28:	2a1903e4 	mov	w4, w25
   91f2c:	911760fa 	add	x26, x7, #0x5d8
...
00000000000927a0 <__swbuf>:
   927a0:	90000023 	adrp	x3, 96000 <JIS_state_table+0xb0>
   927a4:	aa0103e2 	mov	x2, x1
   927a8:	2a0003e1 	mov	w1, w0
   927ac:	f940e060 	ldr	x0, [x3, #448]
   927b0:	17ffffac 	b	92660 <__swbuf_r>
	...

00000000000927c0 <_mbtowc_r>:
   927c0:	90000025 	adrp	x5, 96000 <JIS_state_table+0xb0>
   927c4:	f9469ca5 	ldr	x5, [x5, #3384]
...
0000000000092ce0 <__jis_mbtowc>:
   ...
   92cfc:	f000000c 	adrp	x12, 95000 <pmu_event_descr+0xa0>
   92d00:	f000000b 	adrp	x11, 95000 <pmu_event_descr+0xa0>
   92d04:	aa0003ed 	mov	x13, x0
   92d08:	913c018c 	add	x12, x12, #0xf00
   92d0c:	913d416b 	add	x11, x11, #0xf50
...
0000000000092ec0 <__assert_func>:
   ...
   92ec4:	90000024 	adrp	x4, 96000 <JIS_state_table+0xb0>
   92ec8:	aa0303e5 	mov	x5, x3
   92ecc:	910003fd 	mov	x29, sp
   92ed0:	f940e087 	ldr	x7, [x4, #448]
   92ed4:	aa0003e3 	mov	x3, x0
   92ed8:	aa0203e6 	mov	x6, x2
   92edc:	2a0103e4 	mov	w4, w1
   92ee0:	aa0503e2 	mov	x2, x5
   92ee4:	f0000005 	adrp	x5, 95000 <pmu_event_descr+0xa0>
   92ee8:	f9400ce0 	ldr	x0, [x7, #24]
   92eec:	913e60a5 	add	x5, x5, #0xf98
   92ef0:	b40000a6 	cbz	x6, 92f04 <__assert_func+0x44>
   92ef4:	f0000001 	adrp	x1, 95000 <pmu_event_descr+0xa0>
   92ef8:	913ea021 	add	x1, x1, #0xfa8
...
0000000000092f40 <strcasecmp>:
   92f40:	f0000006 	adrp	x6, 95000 <pmu_event_descr+0xa0>
   92f44:	aa0003e8 	mov	x8, x0
   92f48:	913484c6 	add	x6, x6, #0xd21
...
00000000000930e0 <__multadd>:
   ...
   931d4:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   931d8:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   931dc:	91324063 	add	x3, x3, #0xc90
   931e0:	913f6000 	add	x0, x0, #0xfd8
...
00000000000931f0 <__s2b>:
   ...
   93304:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   93308:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   9330c:	91324063 	add	x3, x3, #0xc90
   93310:	913f6000 	add	x0, x0, #0xfd8
...
0000000000093440 <__i2b>:
   ...
   9346c:	f0000001 	adrp	x1, 96000 <JIS_state_table+0xb0>
   93470:	b9001814 	str	w20, [x0, #24]
   93474:	a94153f3 	ldp	x19, x20, [sp, #16]
   93478:	fd40c020 	ldr	d0, [x1, #384]
   ...
   934a0:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   934a4:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   934a8:	91324063 	add	x3, x3, #0xc90
   934ac:	913f6000 	add	x0, x0, #0xfd8
   ...
   934d0:	f0000001 	adrp	x1, 96000 <JIS_state_table+0xb0>
   934d4:	b9001814 	str	w20, [x0, #24]
   934d8:	a94153f3 	ldp	x19, x20, [sp, #16]
   934dc:	fd40bc20 	ldr	d0, [x1, #376]
   934e0:	f0000001 	adrp	x1, 96000 <JIS_state_table+0xb0>
   934e4:	fd000400 	str	d0, [x0, #8]
   934e8:	fd40c020 	ldr	d0, [x1, #384]
...
0000000000093500 <__multiply>:
   ...
   936c4:	d65f03c0 	ret
   936c8:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   936cc:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   936d0:	91324063 	add	x3, x3, #0xc90
   936d4:	913f6000 	add	x0, x0, #0xfd8
...
00000000000936f0 <__pow5mult>:
   ...
   937ac:	f0000004 	adrp	x4, 96000 <JIS_state_table+0xb0>
   937b0:	9100e084 	add	x4, x4, #0x38
   ...
   937f4:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   937f8:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   937fc:	91324063 	add	x3, x3, #0xc90
   93800:	913f6000 	add	x0, x0, #0xfd8
...
0000000000093810 <__lshift>:
   ...
   9395c:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   93960:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   93964:	91324063 	add	x3, x3, #0xc90
   93968:	913f6000 	add	x0, x0, #0xfd8
...
00000000000939d0 <__mdiff>:
   ...
   93b90:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   93b94:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   93b98:	91324063 	add	x3, x3, #0xc90
   93b9c:	913f6000 	add	x0, x0, #0xfd8
   93ba0:	d2800002 	mov	x2, #0x0                   	// #0
   93ba4:	528046e1 	mov	w1, #0x237                 	// #567
   93ba8:	97fffcc6 	bl	92ec0 <__assert_func>
   93bac:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   93bb0:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   93bb4:	91324063 	add	x3, x3, #0xc90
   93bb8:	913f6000 	add	x0, x0, #0xfd8
...
0000000000093d90 <__d2b>:
   ...
   93ea8:	d0000003 	adrp	x3, 95000 <pmu_event_descr+0xa0>
   93eac:	d0000000 	adrp	x0, 95000 <pmu_event_descr+0xa0>
   93eb0:	91324063 	add	x3, x3, #0xc90
   93eb4:	913f6000 	add	x0, x0, #0xfd8
...
0000000000093f60 <_mprec_log10>:
   ...
   93f7c:	d65f03c0 	ret
   93f80:	f0000001 	adrp	x1, 96000 <JIS_state_table+0xb0>
   93f84:	9102c021 	add	x1, x1, #0xb0
...
00000000000942e0 <_wcsnrtombs_r>:
   942e0:	d0000000 	adrp	x0, 96000 <JIS_state_table+0xb0>
   942e4:	d0000006 	adrp	x6, 96000 <JIS_state_table+0xb0>
   942e8:	913140c6 	add	x6, x6, #0xc50
   942ec:	f940e000 	ldr	x0, [x0, #448]
   942f0:	17ffff90 	b	94130 <_wcsnrtombs_l>
	...

0000000000094300 <wcsnrtombs>:
   94300:	d0000006 	adrp	x6, 96000 <JIS_state_table+0xb0>
   94304:	aa0003e8 	mov	x8, x0
   94308:	aa0103e7 	mov	x7, x1
   9430c:	aa0203e5 	mov	x5, x2
   94310:	f940e0c0 	ldr	x0, [x6, #448]
   ...
   9432c:	d0000006 	adrp	x6, 96000 <JIS_state_table+0xb0>
   94330:	913140c6 	add	x6, x6, #0xc50
...
00000000000943d0 <fiprintf>:
   ...
   943e8:	d0000009 	adrp	x9, 96000 <JIS_state_table+0xb0>
...
00000000000946c0 <raise>:
   ...
   946c4:	d0000001 	adrp	x1, 96000 <JIS_state_table+0xb0>
   946c8:	910003fd 	mov	x29, sp
   946cc:	a90153f3 	stp	x19, x20, [sp, #16]
   946d0:	f940e034 	ldr	x20, [x1, #448]
...
0000000000094800 <_init_signal>:
   ...
   94804:	d0000000 	adrp	x0, 96000 <JIS_state_table+0xb0>
   94808:	910003fd 	mov	x29, sp
   9480c:	f9000bf3 	str	x19, [sp, #16]
   94810:	f940e013 	ldr	x19, [x0, #448]
...
0000000000094860 <__sigtramp>:
   ...
   94864:	d0000001 	adrp	x1, 96000 <JIS_state_table+0xb0>
   94868:	910003fd 	mov	x29, sp
   9486c:	a90153f3 	stp	x19, x20, [sp, #16]
   94870:	f940e034 	ldr	x20, [x1, #448]
...
0000000000094970 <_getpid_r>:
   94970:	17ffb138 	b	80e50 <_getpid>
	...
